...
// Apply permission
if (!$is_previous_year && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_permission'])) {
    ...
    if ($date_timestamp === false) {
        ...
        // Validate that date is within current year range
        if ($formatted_date < $year_start_date || $formatted_date > $year_end_date) {
            $message = '<div class="alert alert-error"><i class="icon-error"></i> Cannot apply for permission outside the current leave year (Mar 16 - Mar 15).</div>';
        }
...
// Cancel permission (for regular pending permissions) - only if not previous year
if (!$is_previous_year && isset($_GET['cancel'])) {
    ...
    if ($perm_data) {
        // Check if this permission belongs to previous year using date range
        if ($perm_data['permission_date'] < $year_start_date || $perm_data['permission_date'] > $year_end_date) {
            $_SESSION['perm_message'] = '<div class="alert alert-error"><i class="icon-error"></i> Cannot cancel permissions from previous years.</div>';
        } else {
            ...
            if ($stmt->execute()) {
                ...
                // NOTIFY HR, ADMIN, AND PM that permission was cancelled
    ...
    header('Location: permissions.php?year=' . urlencode($year_filter));
    exit();
}

// Cancel LOP permission - only if not previous year
if (!$is_previous_year && isset($_GET['cancel_lop'])) {
    ...
    if ($perm_data) {
        ...
        if ($perm_data['permission_date'] < $year_start_date || $perm_data['permission_date'] > $year_end_date) {
            ...
            if ($stmt->execute()) {
                ...
                // NOTIFY HR, ADMIN, AND PM that LOP permission was cancelled
                $notify_users = $conn->query("SELECT id FROM users WHERE role IN ('hr', 'admin', 'pm')");
                while ($notify_user = $notify_users->fetch_assoc()) {
...
// Get user's permissions for selected year - FIXED: Use date range instead of leave_year column
if ($is_previous_year) {
    // For previous year, show permissions within that year's date range
    $stmt = $conn->prepare("
        SELECT * FROM permissions 
        WHERE user_id = ? AND permission_date BETWEEN ? AND ?
        ORDER BY permission_date DESC, applied_date DESC
    ");
    $stmt->bind_param("iss", $user_id, $year_start_date, $year_end_date);
} else {
    // For current year, show permissions within current year's date range
    $stmt = $conn->prepare("
        SELECT * FROM permissions 
        WHERE user_id = ? AND permission_date BETWEEN ? AND ?
        ORDER BY permission_date DESC, applied_date DESC
    ...
    <div class="app-main">
        ...
        <div class="main-content">
            ...
            <div class="archive-note">
                ...
                These records are read-only and cannot be modified. Any actions (cancel) are disabled.
            </div>
            <?php endif; ?>
            ...
            <?php if (!$is_previous_year && $used_hours >= 4): ?>
            ...
            <?php elseif (!$is_previous_year && $remaining_hours > 0 && $remaining_hours < 2): ?>
                        ...
                        <div class="form-group">
                            ...
                            <div class="date-input-container">
                                <input type="date" 
                                       ...
                                       min="<?php echo $year_start_date; ?>"
                                       max="<?php echo $year_end_date; ?>">
                            </div>
                        </div>
                    </div>
                                ...
                                <tr style="<?php echo $is_lop ? 'background:#fff5f5;' : ''; ?>">
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <?php if (!$is_previous_year && $permission['status'] === 'Pending'): ?>
                                                <a href="?cancel=<?php echo $permission['id']; ?>&year=<?php echo urlencode($year_filter); ?>" 
                                            ...
                                            <?php elseif (!$is_previous_year && $permission['status'] === 'LOP'): ?>
                                                <a href="?cancel_lop=<?php echo $permission['id']; ?>&year=<?php echo urlencode($year_filter); ?>" 
                                                   class="btn-small btn-cancel"
                                                   onclick="return confirm('Are you sure you want to cancel this LOP permission?')">
                                                    <i class="icon-cancel"></i> Cancel LOP
                                                </a>
                                            <?php elseif ($is_previous_year && ($permission['status'] === 'Pending' || $permission['status'] === 'LOP')): ?>
                                                <span class="btn-small btn-cancel action-disabled" style="opacity:0.5;" title="Cannot cancel archived permissions">
                                                    <i class="icon-cancel"></i> Cancel
                                                </span>
                                            <?php endif; ?>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="7" style="text-align: center; padding: 40px; color: #718096;">
                                        ...
                                        No permission requests found for <?php echo $year_filter; ?>
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
