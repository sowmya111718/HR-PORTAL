...
// For HR/Admin/dm/COO/ED viewing other users' timesheets
if (isset($_GET['user_id']) && in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed'])) {
...
$year_filter = isset($_GET['year_filter']) ? sanitize($_GET['year_filter']) : 'all';
...
// Check if current user is a management account (exempt from timesheet requirements)
$current_username = $_SESSION['username'];
$is_admin_user = in_array(strtolower($role), ['hr', 'admin', 'dm', 'coo', 'ed', 'mg']) || $user_info['reporting_to'] == 'CEO' || $user_info['position'] == 'CEO';
...
// Ensure num_objects column exists
$conn->query("ALTER TABLE timesheets ADD COLUMN IF NOT EXISTS num_objects INT DEFAULT NULL");
...
// Function to get leave year options (Mar 16 - Mar 15)
function getLeaveYearOptions() {
    // Fixed leave years:
    // Previous year : Mar 16 2025 – Mar 15 2026 (value 2025, label 2025-26)
    // Current year  : Mar 16 2026 – Mar 15 2027 (value 2026, label 2026-27)
    return [
...
// ============================================
// ============================================
// AUTO-GENERATE LOP FOR MISSING TIMESHEETS
// ============================================
// AUTO check: only checks YESTERDAY (1 day) on every page load
// MANUAL check: HR/Admin can trigger check for any date range via POST

// Shared function to run LOP check for a date range
function runLOPCheck($conn, $check_from, $check_to) {
    $users_all = [];
    $ur = $conn->query("SELECT id, username, role, status, created_at, duty_project_id, duty_task_name, duty_hours, duty_software FROM users WHERE status != 'inactive' AND role NOT IN ('hr','admin','dm','coo','ed','MG')");
    while ($u = $ur->fetch_assoc()) { $users_all[] = $u; }
    $ur->close();
    ...
    if (empty($users_all)) return ['added' => 0, 'auto_ts' => 0, 'removed' => 0];

    $user_ids_str = implode(',', array_column($users_all, 'id'));

    // Bulk-load existing auto-LOPs
    $existing_lops = [];
    $lr = $conn->query("SELECT user_id, from_date FROM leaves WHERE leave_type='LOP' AND reason LIKE 'Auto-generated LOP%' AND user_id IN ($user_ids_str) AND from_date BETWEEN '$check_from' AND '$check_to'");
    while ($row = $lr->fetch_assoc()) {
        $existing_lops[$row['user_id'] . '|' . $row['from_date']] = true;
    }
    $lr->close();

    // Bulk-load existing timesheets
    $existing_ts = [];
    $tr = $conn->query("SELECT user_id, entry_date, status FROM timesheets WHERE user_id IN ($user_ids_str) AND entry_date BETWEEN '$check_from' AND '$check_to'");
    while ($row = $tr->fetch_assoc()) {
        $existing_ts[$row['user_id'] . '|' . $row['entry_date']] = $row['status'];
    }
    $tr->close();

    // Bulk-load approved leaves + cleanup exemptions (skip LOP generation for archived dates)
    $approved_leaves = [];
    $al = $conn->query("SELECT user_id, from_date, to_date FROM leaves WHERE status='Approved' AND leave_type != 'LOP' AND user_id IN ($user_ids_str) AND from_date <= '$check_to' AND to_date >= '$check_from'");
    while ($row = $al->fetch_assoc()) {
        $d = new DateTime($row['from_date']);
        $e = new DateTime($row['to_date']);
        while ($d <= $e) {
            $approved_leaves[$row['user_id'] . '|' . $d->format('Y-m-d')] = true;
            $d->modify('+1 day');
        }
    }
    $al->close();

    // Also load cleanup exemptions - dates archived by admin should never get auto-LOP
    $cl = $conn->query("SELECT user_id, from_date FROM leaves WHERE status='Cancelled' AND leave_type='LOP' AND reason LIKE 'CLEANUP-EXEMPTION%' AND user_id IN ($user_ids_str) AND from_date BETWEEN '$check_from' AND '$check_to'");
    while ($row = $cl->fetch_assoc()) {
        $approved_leaves[$row['user_id'] . '|' . $row['from_date']] = true; // treat as exempt
    }
    $cl->close();

    // Cache duty project names
    $duty_project_names = [];
    foreach ($users_all as $u) {
        if ($u['status'] === 'on_duty' && !empty($u['duty_project_id'])) {
            $pid = intval($u['duty_project_id']);
            if (!isset($duty_project_names[$pid])) {
                $ps = $conn->prepare("SELECT project_name, project_code FROM projects WHERE id = ?");
                $ps->bind_param("i", $pid);
                $ps->execute();
                $pr = $ps->get_result()->fetch_assoc();
                $ps->close();
                $pname = $pr ? $pr['project_name'] : '';
                if ($pr && !empty($pr['project_code'])) $pname .= ' [' . $pr['project_code'] . ']';
                $duty_project_names[$pid] = $pname;
            }
        }
    }

    // Loop dates × users
    $start_date = new DateTime($check_from);
    $end_date   = new DateTime($check_to);
    $end_date->modify('+1 day');
    $date_range = new DatePeriod($start_date, new DateInterval('P1D'), $end_date);

    foreach ($date_range as $date_obj) {
        $check_date = $date_obj->format('Y-m-d');
        if (date('N', strtotime($check_date)) == 7) continue; // Skip Sundays
        if (isHoliday($check_date) !== false) continue;       // Skip holidays

        foreach ($users_all as $user) {
            if (!empty($user['created_at']) && $check_date < date('Y-m-d', strtotime($user['created_at']))) continue;

            $key        = $user['id'] . '|' . $check_date;
            $lop_exists = isset($existing_lops[$key]);
            $ts_status  = $existing_ts[$key] ?? null;
            $has_leave  = isset($approved_leaves[$key]);

            if ($lop_exists && $ts_status !== 'approved') continue;

            if (!$ts_status && !$has_leave) {
                if ($user['status'] === 'on_duty') {
                    $duty_project_id   = $user['duty_project_id'] ?? null;
                    $duty_project_name = $duty_project_id ? ($duty_project_names[intval($duty_project_id)] ?? '') : '';
                    $duty_hours        = floatval($user['duty_hours'] ?? 8);
                    $duty_task         = $user['duty_task_name'] ?? 'On Duty';
                    $duty_software_val = $user['duty_software'] ?? 'Other';
                    $duty_remarks      = 'Auto-submitted (On Duty)';
                    $duty_status_val   = 'completed';
                    $submitted_date    = $check_date . ' 23:59:00';

                    $ins = $conn->prepare("INSERT INTO timesheets (user_id, entry_date, hours, software, project_id, project_name, task_name, remarks, status, submitted_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $ins->bind_param("isdsisssss", $user['id'], $check_date, $duty_hours, $duty_software_val, $duty_project_id, $duty_project_name, $duty_task, $duty_remarks, $duty_status_val, $submitted_date);
                    if ($ins->execute()) {
                        $auto_ts_count++;
                        $existing_ts[$key] = 'completed';
                        if ($lop_exists) {
                            $uid = intval($user['id']);
                            $conn->query("DELETE FROM leaves WHERE user_id=$uid AND from_date='$check_date' AND leave_type='LOP' AND reason LIKE 'Auto-generated LOP%'");
                            $lop_removed_count++;
                            unset($existing_lops[$key]);
                        }
                    }
                    $ins->close();
                } else {
                    if (!$lop_exists) {
                        if (addLOPForMissingTimesheet($conn, $user['id'], $check_date)) {
                            $lop_added_count++;
                            $existing_lops[$key] = true;
                        }
                    }
                }
            } elseif ($ts_status === 'approved' && $lop_exists) {
                $uid = intval($user['id']);
                $conn->query("DELETE FROM leaves WHERE user_id=$uid AND from_date='$check_date' AND leave_type='LOP' AND reason LIKE 'Auto-generated LOP%'");
                $lop_removed_count++;
                unset($existing_lops[$key]);
    ...
    return ['added' => $lop_added_count, 'auto_ts' => $auto_ts_count, 'removed' => $lop_removed_count];
}
...
if (in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed'])) {
    ...
    // ── MANUAL CHECK: triggered by HR/Admin via form ──────────────────────
    if (isset($_POST['manual_lop_check']) && isset($_POST['lop_from_date'], $_POST['lop_to_date'])) {
        $manual_from = sanitize($_POST['lop_from_date']);
        $manual_to   = sanitize($_POST['lop_to_date']);
        if ($manual_from > $manual_to) { $t = $manual_from; $manual_from = $manual_to; $manual_to = $t; }
        // Ensure not checking today or future
        if ($manual_to >= $today) $manual_to = $yesterday;

        $result = runLOPCheck($conn, $manual_from, $manual_to);
        $msgs = [];
        if ($result['added']   > 0) $msgs[] = $result['added']   . ' new LOP entries added';
        if ($result['auto_ts'] > 0) $msgs[] = $result['auto_ts'] . ' auto-timesheets submitted for On Duty users';
        if ($result['removed'] > 0) $msgs[] = $result['removed'] . ' LOP entries removed (approved timesheets found)';
        $message = '<div class="alert alert-' . (empty($msgs) ? 'success' : 'info') . '"><i class="icon-info"></i> Manual LOP check (' . $manual_from . ' to ' . $manual_to . '): ' . (empty($msgs) ? 'No changes needed — all up to date.' : implode(', ', $msgs)) . '</div>';

    } else {
        // ── AUTO CHECK: only yesterday ────────────────────────────────────────
        $result = runLOPCheck($conn, $yesterday, $yesterday);
        if ($result['added'] > 0 || $result['auto_ts'] > 0 || $result['removed'] > 0) {
            $msgs = [];
            if ($result['added']   > 0) $msgs[] = $result['added']   . ' new LOP entries added';
            if ($result['auto_ts'] > 0) $msgs[] = $result['auto_ts'] . ' auto-timesheets submitted for On Duty users';
            if ($result['removed'] > 0) $msgs[] = $result['removed'] . ' LOP entries removed (approved timesheets found)';
            $message = '<div class="alert alert-info"><i class="icon-info"></i> Auto-check (yesterday): ' . implode(', ', $msgs) . '.</div>';
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_timesheet'])) {
    ...
    $num_objects = isset($_POST['num_objects']) && $_POST['num_objects'] !== '' ? intval($_POST['num_objects']) : null;
    ...
    if ($approved_leave) {
        ...
        if ($check_result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("
                INSERT INTO timesheets (user_id, entry_date, hours, software, project_id, project_name, task_id, task_name, remarks, num_objects, status, submitted_date)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->bind_param("isdssisssiss", $viewing_user_id, $date, $total_hours, $software, $project_id, $project_name, $task_id, $task_name, $remarks, $num_objects, $status, $submitted_date);
            
            if ($stmt->execute()) {
                $timesheet_id = $stmt->insert_id;
                
                // If this is for a past date and there was an auto LOP, remove it
                if ($date < date('Y-m-d')) {
                    $remove_lop = $conn->prepare("DELETE FROM leaves WHERE user_id = ? AND from_date = ? AND leave_type = 'LOP' AND reason LIKE 'Auto-generated LOP%'");
                    $remove_lop->bind_param("is", $viewing_user_id, $date);
                    $remove_lop->execute();
                    $remove_lop->close();
                }
                ...
                $redirect_url = "timesheet.php?from_date=" . urlencode($date) . "&to_date=" . urlencode($date) . "&year_filter=" . urlencode($year_filter);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_timesheet'])) {
    ...
    if ($owner_data && $owner_data['user_id'] == $user_id && $owner_data['status'] != 'approved') {
        if ($owner_data['entry_date'] == $today_date) {
            $stmt = $conn->prepare("
                UPDATE timesheets 
                SET entry_date = ?, hours = ?, software = ?, project_id = ?, project_name = ?, task_id = ?, task_name = ?, remarks = ?, num_objects = ?, status = ?, submitted_date = ?
                WHERE id = ?
            ");
            $stmt->bind_param("sdsissssissi", $date, $total_hours, $software, $project_id, $project_name, $task_id, $task_name, $remarks, $num_objects, $status, $current_time, $edit_id);
            ...
            if ($stmt->execute()) {
                ...
                $redirect_url = "timesheet.php?from_date=" . urlencode($from_date) . "&to_date=" . urlencode($to_date) . "&year_filter=" . urlencode($year_filter);
                if (isset($_GET['user_id']) && $_GET['user_id'] != '') {
                    $redirect_url .= "&user_id=" . urlencode($_GET['user_id']);
                }
                header("Location: $redirect_url");
...
// dm/COO/ED Approval action - WITH NOTIFICATION FOR EMPLOYEE (only for late entries)
if (isset($_GET['approve']) && in_array($role, ['dm', 'ed'])) {
    ...
    if ($ts_data) {
        // Check if this is a late entry, Sunday, or festival/holiday day
        $is_late = isLateTimesheet($ts_data['entry_date'], $ts_data['submitted_date']);
        $is_sunday_entry = (date('N', strtotime($ts_data['entry_date'])) == 7);
        $is_festival_entry = (isHoliday($ts_data['entry_date']) !== false);

        if (!$is_late && !$is_sunday_entry && !$is_festival_entry) {
            $message = '<div class="alert alert-error"><i class="icon-error"></i> Approval is only allowed for late timesheet submissions, Sunday work, or festival/holiday day work.</div>';
        } else {
            ...
            if ($update->execute()) {
                ...
                if ($is_festival_entry) {
                    $casual_added = addCasualLeaveForFestivalWork($conn, $ts_data['user_id'], $ts_data['entry_date'], $user_id);
                    $message = $casual_added
                        ? '<div class="alert alert-success"><i class="icon-success"></i> Festival day timesheet approved. +1 Casual Leave granted to employee. Notification sent.</div>'
                        : '<div class="alert alert-success"><i class="icon-success"></i> Festival day timesheet approved. Notification sent to employee.</div>';
                } elseif ($is_sunday) {
                    $casual_added = addCasualLeaveForSundayWork($conn, $ts_data['user_id'], $ts_data['entry_date'], $user_id);
                    $message = $casual_added
                        ? '<div class="alert alert-success"><i class="icon-success"></i> Timesheet approved successfully. LOP removed and 1 casual leave added. Notification sent to employee.</div>'
                        : '<div class="alert alert-success"><i class="icon-success"></i> Timesheet approved successfully. LOP removed. Notification sent to employee.</div>';
                } else {
                    $message = $lop_removed
                        ? '<div class="alert alert-success"><i class="icon-success"></i> Timesheet approved successfully. Auto-generated LOP has been removed. Notification sent to employee.</div>'
                        : '<div class="alert alert-success"><i class="icon-success"></i> Timesheet approved successfully. Notification sent to employee.</div>';
                }
...
} elseif (isset($_GET['approve']) && $role == 'admin') {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> Admins cannot approve timesheets. Use dm or ED.</div>';
} elseif (isset($_GET['approve']) && ($role == 'hr' || $role == 'coo')) {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> HR/COO cannot approve timesheets. Use dm or ED.</div>';
}

// dm/COO/ED Rejection action - WITH NOTIFICATION FOR EMPLOYEE (only for late entries)
if (isset($_GET['reject']) && in_array($role, ['dm', 'ed'])) {
    ...
    if ($ts_data) {
        // Check if this is a late entry or festival/holiday day
        $is_late = isLateTimesheet($ts_data['entry_date'], $ts_data['submitted_date']);
        $is_festival_entry = (isHoliday($ts_data['entry_date']) !== false);

        if (!$is_late && !$is_festival_entry) {
            $message = '<div class="alert alert-error"><i class="icon-error"></i> Rejection is only allowed for late timesheet submissions or festival/holiday day work.</div>';
        } else {
...
} elseif (isset($_GET['reject']) && $role == 'admin') {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> Admins cannot reject timesheets. Use dm or ED.</div>';
} elseif (isset($_GET['reject']) && ($role == 'hr' || $role == 'coo')) {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> HR/COO cannot reject timesheets. Use dm or ED.</div>';
}

// Delete timesheet entry - Only for dm, COO, ED (NOT Admin)
if (isset($_GET['delete']) && in_array($role, ['dm', 'ed'])) {
    ...
    if (!$timesheet_user_id || !$entry_date) {
        $message = '<div class="alert alert-error"><i class="icon-error"></i> Timesheet entry not found.</div>';
    } else {
        ...
        try {
            ...
            // Notify all HR/Admin/dm/COO/ED about the deletion
            $mgmt_users = $conn->query("SELECT id FROM users WHERE role IN ('hr', 'admin', 'dm', 'coo', 'ed') AND id NOT IN ($timesheet_user_id, $user_id)");
...
} elseif (isset($_GET['delete']) && $role == 'admin') {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> Admins cannot delete timesheet entries. Use dm or ED.</div>';
} elseif (isset($_GET['delete']) && (strtolower($role) == 'hr' || strtolower($role) == 'coo')) {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> HR/COO cannot delete timesheet entries. Use dm or ED.</div>';
} elseif (isset($_GET['delete']) && !in_array(strtolower($role), ['dm', 'ed', 'admin', 'hr', 'coo'])) {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> You do not have permission to delete timesheet entries.</div>';
}

// Apply year filter based on leave year (Mar 16 - Mar 15)
if ($year_filter != 'all' && is_numeric($year_filter)) {
    $from_date = $year_filter . '-03-16';
    $to_date = ($year_filter + 1) . '-03-15';
}
...
// ============================================
// GET LOP ENTRIES FOR THE DATE RANGE
// ============================================
$lop_entries = [];
if (in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed'])) {
    if (isset($_GET['user_id']) && $_GET['user_id'] != '') {
        $lop_query = $conn->prepare("
            SELECT l.*, u.full_name as employee_name, u.username, u.department, u.position,
                   l.from_date as entry_date, 'LOP' as status, l.days as hours
            FROM leaves l
        ...
        ");
        $lop_query->bind_param("ssi", $from_date, $to_date, $viewing_user_id);
    } else {
        $lop_query = $conn->prepare("
            SELECT l.*, u.full_name as employee_name, u.username, u.department, u.position,
                   l.from_date as entry_date, 'LOP' as status, l.days as hours
        ...
        ");
        $lop_query->bind_param("ss", $from_date, $to_date);
    }
    $lop_query->execute();
    $lop_entries_result = $lop_query->get_result();
    $lop_entries = $lop_entries_result->fetch_all(MYSQLI_ASSOC);
    $lop_query->close();
}
...
// Software options
$software_options = ['Cyclone', 'Revit(cad to bim)','Revit(scan to bim)' ,'SP3D', 'Plant 3D', 'AutoCAD', 'HR', 'Admin', 'Other', 'Training'];

// Set page title
$page_title = 'Timesheet';
if (in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed'])) {
    if ($role === 'dm') {
        $page_title = 'Delivery Manager - Timesheet Overview';
    } elseif ($role === 'hr') {
        $page_title = 'HR - Timesheet Overview';
    } elseif ($role === 'admin') {
        $page_title = 'Admin - Timesheet Overview';
    } elseif ($role === 'coo') {
        $page_title = 'COO - Timesheet Overview';
    } elseif ($role === 'ed') {
        $page_title = 'ED - Timesheet Overview';
    }
}

// Get selected user display name
$selected_user_name = 'All Employees';
if (isset($_GET['user_id']) && $_GET['user_id'] != '' && in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed'])) {
    foreach ($users as $u) {
...
// ============================================
if (isset($_GET['export_excel']) && in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed'])) {
    ob_start(); // Buffer all output so headers can be sent cleanly
    ...
    // Clear any previous output before sending file headers
    ...
    foreach ($all_users as $user) {
        ...
        if (isset($timesheets_data[$user_key])) {
            foreach ($timesheets_data[$user_key] as $date => $entry) {
                ...
                if (date('N', strtotime($date)) == 7) {
                    // Sunday: only count if approved by dm/COO/ED
                    if ($entry['status'] === 'approved') {
...
$today_date = date('Y-m-d');
$leave_year_options = getLeaveYearOptions();
...
<head>
    ...
    <style>
        ...
        .timesheet-row { display: grid; grid-template-columns: 1fr 0.8fr 1.2fr 1.5fr 1.5fr 1.2fr 1fr 0.8fr; gap: 15px; margin-bottom: 20px; padding-bottom: 20px; border-bottom: 1px solid #e2e8f0; }
        ...
        .lop-entry { background-color: #fff5f5 !important; } /* Light red background */
        .lop-entry td { color: #c53030; }
        ...
        .icon-timesheet:before { content: "\1F4DD "; }
        ...
        .icon-project:before { content: "\1F4C1 "; }
        .icon-task:before { content: "\1F4CB "; }
        .icon-list:before { content: "\1F4DD "; }
        .icon-software:before { content: "\1F4BB "; }
        .icon-clock:before { content: "\23F0 "; }
        .icon-calendar:before { content: "\1F4C5 "; }
        .icon-excel:before { content: "\1F4E4 "; }
        .icon-settings:before { content: "\2699 "; }
        .icon-user-tie:before { content: "\1F454 "; }
        
        /* Task specific icons */
        .task-bolt:before { content: "\26A1 "; }
        .task-cable:before { content: "\1F50C "; }
        .task-wrench:before { content: "\1F6E0 "; }
        .task-railing:before { content: "\1F6A7 "; }
        .task-building:before { content: "\1F3DB "; }
        .task-stairs:before { content: "\2581\2583\2585\2587 "; }
        .task-cogs:before { content: "\2699 "; }
        .task-pipe:before { content: "\1F527 "; }
        .task-cubes:before { content: "\1F3D7 "; }
        .task-hvac:before { content: "\2744 "; }
        
        /* Status icons */
        .status-approved:before { content: "\2705 "; }
        .status-pending:before { content: "\23F3 "; }
        .status-error:before { content: "\26A0 "; }
        .status-holiday:before { content: "\1F389 "; }
        .status-leave:before { content: "\1F3D6 "; }
    </style>
</head>
<body>
    ...
    <div class="app-main">
        ...
        <div class="main-content">
            <h2 class="page-title">
                ...
                <?php echo $page_title; ?>
                <?php if (in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed']) && !isset($_GET['user_id'])): ?>
            ...
            <!-- Manual LOP Check Panel -->
            <div style="background: #ebf8ff; border: 1px solid #90cdf4; border-radius: 10px; padding: 15px; margin-bottom: 20px;">
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
                    <i class="icon-sync" style="color: #2b6cb0;"></i>
                    <strong style="color: #2b6cb0;">Manual LOP Check</strong>
                    <span style="color: #718096; font-size: 12px;">Auto-check runs for yesterday only on each page load. Use this to check any past date range.</span>
                </div>
                <form method="POST" action="" style="display: flex; gap: 10px; align-items: flex-end; flex-wrap: wrap;">
                    <div>
                        <label style="display: block; font-size: 12px; font-weight: 600; color: #4a5568; margin-bottom: 4px;">From Date</label>
                        <input type="date" name="lop_from_date" class="form-control" style="width: 160px;" 
                               value="<?php echo date('Y-m-d', strtotime('-7 days')); ?>"
                               max="<?php echo date('Y-m-d', strtotime('-1 day')); ?>" required>
                    </div>
                    <div>
                        <label style="display: block; font-size: 12px; font-weight: 600; color: #4a5568; margin-bottom: 4px;">To Date</label>
                        <input type="date" name="lop_to_date" class="form-control" style="width: 160px;"
                               value="<?php echo date('Y-m-d', strtotime('-1 day')); ?>"
                               max="<?php echo date('Y-m-d', strtotime('-1 day')); ?>" required>
                    </div>
                    <button type="submit" name="manual_lop_check" class="btn" style="background: #2b6cb0; color: white; padding: 8px 20px;">
                        <i class="icon-sync"></i> Run LOP Check
                    </button>
                </form>
            </div>
            ...
            <div class="project-management" style="background: #fed7d7; border-color: #fc8181;">
                ...
                <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                    ...
                    <span style="color: #718096; font-size: 12px;">LOPs are automatically generated when you open the portal</span>
                </div>
            </div>
                    ...
                    <div>
                        ...
                        <div style="display: flex; gap: 10px; margin-top: 5px;">
                            <span class="role-badge">
                                ...
                                elseif ($role == 'dm') echo 'Delivery Manager';
                                elseif ($role == 'coo') echo 'Chief Operating Officer';
                                elseif ($role == 'ed') echo 'Executive Director';
                                ?>
                            </span>
                        </div>
                    </div>
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                        ...
                        <form method="GET" action="" id="filterForm" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                            <input type="hidden" name="view_type" value="<?php echo isset($_GET['view_type']) ? $_GET['view_type'] : ''; ?>">
                ...
                <div style="margin-bottom: 20px; padding: 15px; background: #f7fafc; border-radius: 10px;">
                    <div style="display: flex; gap: 20px; margin-bottom: 10px; flex-wrap: wrap;">
                        ...
                        <?php endif; ?>
                      <?php
                      // Calculate hours and minutes from total_hours
                      $total_whole_hours = floor($total_hours);
                      $total_minutes = round(($total_hours - $total_whole_hours) * 60);
                      $total_display = $total_whole_hours . 'h ' . $total_minutes . 'm';
                      ?>
                      <div>
                          <strong>Total Hours <?php echo in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed']) ? '(' . $from_date . ' to ' . $to_date . ')' : 'Today'; ?>:</strong> 
                          <span style="font-weight: bold; color: #276749;"><?php echo $total_display; ?></span>
                          <?php if ($working_days_in_range > 0 && in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed'])): ?>
                ...
                <div class="timesheet-form" style="border: 2px solid #4299e1;">
                    ...
                    <form method="POST" action="" id="timesheetForm">
                        <div class="timesheet-row">
                                ...
                                <div id="task_dropdown_container" style="<?php echo (!empty($edit_data['task_name']) && strpos($edit_data['task_name'], ',') !== false) ? 'display: none;' : 'display: block;'; ?>">
                                    <div class="task-checkbox-group" id="multi_task_container">
                                        <?php if (!empty($all_tasks)): ?>
                                            ...
                                            elseif  (strpos($task['task_name'], 'Cable Tray')   !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Support')      !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Railing')      !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Architecture') !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Stair')        !== false) $icon = '▁▃▅▇ ';
                                            elseif  (strpos($task['task_name'], 'Equipment')    !== false) $icon = '⚙';
                                            elseif  (strpos($task['task_name'], 'Piping')       !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Structure')    !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'HVAC')         !== false) $icon = '❄';
                                            elseif  (strpos($task['task_name'], 'Civil')        !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Structural')   !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Mechanical')   !== false) $icon = '⚙';
                                            elseif  (strpos($task['task_name'], 'Plumbing')     !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Instrumentation') !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Fire')         !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Road')         !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Telecom')      !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'QC')           !== false) $icon = '✅';
                                            elseif  (strpos($task['task_name'], 'Quality')      !== false) $icon = '✅';
                                            elseif  (strpos($task['task_name'], 'Design')       !== false) $icon = '✏';
                                            elseif  (strpos($task['task_name'], 'Drawing')      !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Review')       !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Report')       !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Meeting')      !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Admin')        !== false) $icon = '';
                                            elseif  (strpos($task['task_name'], 'Other')        !== false) $icon = '';
                                            ?>
                            ...
                            <div>
                                <label class="form-label">🔢 Number of Objects</label>
                                <input type="number" name="num_objects" class="form-control" placeholder="e.g. 10" min="0" value="<?php echo htmlspecialchars($edit_data['num_objects'] ?? ''); ?>">
                            </div>
                            <div>
                        ...
                        <div style="display: flex; align-items: center;">
                            ...
                            <a href="?cancel_edit=1&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year_filter=<?php echo $year_filter; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>" class="btn-cancel">
                ...
                <!-- Add Timesheet Form -->
                <?php elseif (!$existing_timesheet && (!in_array(strtolower($role), ['hr', 'admin', 'dm', 'coo', 'ed', 'mg']) || isset($_GET['user_id']))): 
                ...
                <div class="timesheet-form">
                    ...
                    <form method="POST" action="" id="timesheetForm">
                        <div class="timesheet-row">
                            <div>
                                <label class="form-label"><span class="icon-calendar"></span> Date *</label>
                                <input type="date" name="date" class="form-control" value="<?php echo $from_date; ?>" onchange="changeDate(this.value)" required>
                            </div>
                            <div>
                                <label class="form-label"><span class="icon-clock"></span> Hours *</label>
                                <div class="time-input">
                            ...
                            <div>
                                <label class="form-label">🔢 Number of Objects</label>
                                <input type="number" name="num_objects" class="form-control" placeholder="e.g. 10" min="0">
                            </div>
                            <div>
                                <label class="form-label"><span class="icon-flag">🏷️</span> Status *</label>
                                <select name="status" class="form-control" required>
                ...
                <?php endif; ?>
                <?php elseif ($existing_timesheet && (!in_array(strtolower($role), ['hr', 'admin', 'dm', 'coo', 'ed', 'mg']) || isset($_GET['user_id'])) && !$edit_mode): ?>
                <div class="existing-entry">
                    <i class="icon-check"></i>
                    <div>
                        <strong>Timesheet is submitted for <?php echo $from_date; ?></strong><br>
                        <?php echo $user_info['full_name'] ?? $selected_user_name; ?> has submitted a timesheet for this date. Only one entry per day is allowed.
                    </div>
                </div>
                <?php endif; ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php if ($timesheets && $timesheets->num_rows > 0): ?>
                                ...
                                $can_delete = in_array($role, ['dm', 'ed']) && ($entry['id'] > 0) && ($entry['status'] != 'approved');
                                ...
                                elseif (strpos($display_task_name, 'Cable Tray')   !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Support')      !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Railing')      !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Architecture') !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Stair')        !== false) $task_icon = '▁▃▅▇ ';
                                elseif (strpos($display_task_name, 'Equipment')    !== false) $task_icon = '⚙';
                                elseif (strpos($display_task_name, 'Piping')       !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Structure')    !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'HVAC')         !== false) $task_icon = '❄';
                                elseif (strpos($display_task_name, 'Civil')        !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Mechanical')   !== false) $task_icon = '⚙';
                                elseif (strpos($display_task_name, 'Plumbing')     !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Instrumentation') !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Fire')         !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Road')         !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Telecom')      !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'QC')           !== false) $task_icon = '✅';
                                elseif (strpos($display_task_name, 'Quality')      !== false) $task_icon = '✅';
                                elseif (strpos($display_task_name, 'Design')       !== false) $task_icon = '✏';
                                elseif (strpos($display_task_name, 'Drawing')      !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Review')       !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Report')       !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Meeting')      !== false) $task_icon = '';
                                elseif (strpos($display_task_name, 'Admin')        !== false) $task_icon = '';
                                else $task_icon = '';
                                // For comma-separated multi-tasks, build per-task icons
                                if (strpos($display_task_name, ',') !== false) {
                                    $task_parts = array_map('trim', explode(',', $display_task_name));
                                    $task_with_icons = [];
                                    foreach ($task_parts as $tp) {
                                        $ti = '';
                                        if      (strpos($tp, 'Electrical')      !== false) $ti = '⚡';
                                        elseif  (strpos($tp, 'Cable Tray')      !== false) $ti = '';
                                        elseif  (strpos($tp, 'Support')         !== false) $ti = '';
                                        elseif  (strpos($tp, 'Railing')         !== false) $ti = '';
                                        elseif  (strpos($tp, 'Architecture')    !== false) $ti = '';
                                        elseif  (strpos($tp, 'Stair')           !== false) $ti = '▁▃▅▇ ';
                                        elseif  (strpos($tp, 'Equipment')       !== false) $ti = '⚙';
                                        elseif  (strpos($tp, 'Piping')          !== false) $ti = '';
                                        elseif  (strpos($tp, 'Structure')       !== false) $ti = '';
                                        elseif  (strpos($tp, 'HVAC')            !== false) $ti = '❄';
                                        elseif  (strpos($tp, 'Civil')           !== false) $ti = '';
                                        elseif  (strpos($tp, 'Mechanical')      !== false) $ti = '⚙';
                                        elseif  (strpos($tp, 'Plumbing')        !== false) $ti = '';
                                        elseif  (strpos($tp, 'Instrumentation') !== false) $ti = '';
                                        elseif  (strpos($tp, 'Fire')            !== false) $ti = '';
                                        elseif  (strpos($tp, 'Road')            !== false) $ti = '';
                                        elseif  (strpos($tp, 'Telecom')         !== false) $ti = '';
                                        elseif  (strpos($tp, 'QC')              !== false) $ti = '✅';
                                        elseif  (strpos($tp, 'Quality')         !== false) $ti = '✅';
                                        elseif  (strpos($tp, 'Design')          !== false) $ti = '✏';
                                        elseif  (strpos($tp, 'Drawing')         !== false) $ti = '';
                                        elseif  (strpos($tp, 'Review')          !== false) $ti = '';
                                        elseif  (strpos($tp, 'Report')          !== false) $ti = '';
                                        elseif  (strpos($tp, 'Meeting')         !== false) $ti = '';
                                        elseif  (strpos($tp, 'Admin')           !== false) $ti = '';
                                        $task_with_icons[] = $ti . ' ' . $tp;
                                    }
                                    $display_task_name = implode(', ', $task_with_icons);
                                    $task_icon = ''; // icons already embedded per task
                                }
                                ?>
                                <tr class="<?php 
                                ...
                                ?>">
                                    ...
                                    <td style="text-align:center; font-weight:600; color:#4a5568;">
                                        <?php echo isset($entry['num_objects']) && $entry['num_objects'] !== null ? htmlspecialchars($entry['num_objects']) : '-'; ?>
                                    </td>
                                    <td>
                                        <?php if ($is_approved): ?>
                                    ...
                                    <td>
                                        <div class="action-buttons" style="display: flex; gap: 5px; flex-wrap: wrap;">
                                            ...
                                            <a href="?edit=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year_filter=<?php echo $year_filter; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>" 
                                            ...
                                            <?php 
                                            $can_delete_late     = in_array($role, ['dm', 'ed']) && $is_late && !$is_holiday;
                                            $can_delete_regular  = in_array($role, ['dm', 'ed']) && !$is_approved && !$is_holiday;
                                            $can_delete_sunday   = in_array($role, ['dm', 'ed']) && $is_sunday && !$is_holiday;
                                            $can_delete_festival = in_array($role, ['dm', 'ed']) && $is_holiday && !$is_approved;
                                            
                                            if ($can_delete_late || $can_delete_regular || $can_delete_sunday || $can_delete_festival): 
                                            ?>
                                                <?php if ($is_sunday && $has_sunday_leave): ?>
                                                <div class="delete-options">
                                                    <a href="?delete=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year_filter=<?php echo $year_filter; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>" 
                                                       class="btn-small btn-delete"
                                                       onclick="return confirm('Delete timesheet only? The associated Sunday leave will remain.')">
                                                        <i class="icon-delete"></i> TS Only
                                                    </a>
                                                    <a href="?delete=<?php echo $entry['id']; ?>&delete_sunday_leave=1&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year_filter=<?php echo $year_filter; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>" 
                                            ...
                                            <?php if (in_array($role, ['dm', 'ed']) && !$is_approved && $is_holiday): ?>
                                            <a href="?approve=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year_filter=<?php echo $year_filter; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>" 
                                               class="btn-small btn-approve" style="background:#9b59b6;"
                                               onclick="return confirm('Approve festival day timesheet? Employee will get +1 Casual Leave.')">
                                                <i class="icon-check"></i> +1 CL
                                            </a>
                                            <a href="?reject=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year_filter=<?php echo $year_filter; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>" 
                                               class="btn-small btn-reject" style="background:#f56565;"
                                               onclick="return confirm('Reject this festival day timesheet?')">
                                                <i class="icon-cancel"></i> Reject
                                            </a>
                                            <?php elseif (in_array($role, ['dm', 'ed']) && !$is_approved && $is_late && !$is_holiday): ?>
                                            <a href="?approve=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year_filter=<?php echo $year_filter; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>" 
                                            ...
                                            <?php elseif (in_array($role, ['admin']) && !$is_approved && ($is_late || $is_holiday)): ?>
                                            <span class="btn-small btn-approve-disabled" title="Admin cannot approve">Approve</span>
                                            <span class="btn-small btn-reject-disabled" title="Admin cannot reject">Reject</span>
                                            <?php endif; ?>

                                            <?php if (in_array($role, ['dm', 'ed']) && !$is_approved && $is_sunday && !$is_holiday): ?>
                                            <a href="?approve=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year_filter=<?php echo $year_filter; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>" 
                                            ...
                                            <?php elseif ($is_approved && $is_holiday): ?>
                                            <span class="btn-small" style="background: #9b59b6; color: white;">
                                                <i class="icon-check"></i> +1 CL
                                            </span>
                            ...
                            <!-- Display LOP entries as separate rows with date on one line -->
                            <?php if (!empty($lop_entries)): ?>
                                <?php foreach ($lop_entries as $lop): ?>
                                <tr class="lop-entry">
                                    <td><?php echo $lop['entry_date']; ?></td>
                                    <td>0h 0m</td>
                                    <td>-</td>
                                    <td>-</td>
                                    <td>-</td>
                                    <td><?php echo htmlspecialchars($lop['reason']); ?></td>
                                    <td>
                                        <span class="status-badge" style="background: #c53030; color: white;">⚠️ Auto LOP</span>
                                    </td>
                                    <?php if (in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed'])): ?>
                                    <td><strong><?php echo $lop['employee_name']; ?></strong></td>
                                    <?php endif; ?>
                                    <td>-</td>
                                    <td>
                                        <div class="action-buttons">
                                            <span class="btn-small" style="background: #c53030; color: white; cursor: default;">
                                                <i class="icon-info"></i> Auto LOP
                                            </span>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="status-badge" style="background: #c53030; color: white;">
                                            ⚠️ Missing Timesheet
                                        </span>
                                    </td>
                                </tr>
                            ...
                            <?php if ($timesheets->num_rows == 0 && empty($lop_entries)): ?>
                                <tr>
                                    <td colspan="<?php 
                                        $colspan = 10;
                                        if (in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed'])) $colspan += 1;
                                        echo $colspan;
                                    ?>" style="text-align: center; padding: 40px;">
                                        <div class="no-timesheet">
                                            <i class="icon-clock"></i>
                                            <div>No timesheet entries or LOP records found for <?php echo $from_date; ?><?php echo ($from_date != $to_date) ? ' to ' . $to_date : ''; ?></div>
                                            <p style="margin-top: 10px; color: #718096;">
                                                <i class="icon-info"></i> 
                                                <?php if (in_array($role, ['hr', 'admin', 'dm', 'coo', 'ed']) && !isset($_GET['user_id'])): ?>
                                                    No employees have submitted timesheets or have LOP entries for this date range.
                                                <?php else: ?>
                ...
                function changeUser(userId) {
                    ...
                    const yearFilter = '<?php echo $year_filter; ?>';
                    if (fromDate && toDate) {
                        if (userId) {
                            window.location.href = `timesheet.php?user_id=${userId}&from_date=${fromDate}&to_date=${toDate}&year_filter=${yearFilter}`;
                        } else {
                            window.location.href = `timesheet.php?from_date=${fromDate}&to_date=${toDate}&year_filter=${yearFilter}`;
                        }
                    }
                }
                ...
                function changeDate(date) {
                    ...
                    params.set('year_filter', yearFilter);
                    if (userId) params.set('user_id', userId);
                    window.location.href = 'timesheet.php?' + params.toString();
                }
