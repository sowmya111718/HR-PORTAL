...
// Get date range parameters (within the selected year)
...
// Ensure dates are within the selected year range
...
if (in_array($role, ['hr', 'admin', 'pm']) && isset($_GET['check_missing'])) {
    ...
    // Skip if checking date is outside current year
    if ($check_date < $year_start_date || $check_date > $year_end_date) {
        $message = '<div class="alert alert-info"><i class="icon-info"></i> Skipped check for ' . $check_date . ' (outside selected year range).</div>';
    }
    // Skip if checking date is Sunday
    else if (date('N', strtotime($check_date)) == 7) {
...
if (!$is_previous_year && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_timesheet'])) {
    $date = sanitize($_POST['date']);
    $hours = floatval($_POST['hours']);
    $minutes = intval($_POST['minutes']);
    
    // Check if date is within current year range
    if ($date < $year_start_date || $date > $year_end_date) {
        $message = '<div class="alert alert-error"><i class="icon-error"></i> Cannot submit timesheet for dates outside the current leave year.</div>';
    } else {
        ...
        if ($approved_leave) {
            ...
            if ($check_result->num_rows > 0) {
                ...
                if ($stmt->execute()) {
                    ...
                    $redirect_url = "timesheet.php?from_date=" . urlencode($date) . "&to_date=" . urlencode($date) . "&year=" . urlencode($year_filter);
...
if (!$is_previous_year && isset($_GET['edit'])) {
    ...
    if ($edit_entry && $edit_entry['user_id'] == $user_id && $edit_entry['status'] != 'approved') {
        // Check if entry is within current year
        if ($edit_entry['entry_date'] < $year_start_date || $edit_entry['entry_date'] > $year_end_date) {
            $message = '<div class="alert alert-error"><i class="icon-error"></i> Cannot edit timesheet entries from previous years.</div>';
        } elseif ($edit_entry['entry_date'] == $today_date) {
...
if (!$is_previous_year && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_timesheet'])) {
    ...
    // Check if date is within current year range
    if ($date < $year_start_date || $date > $year_end_date) {
        $message = '<div class="alert alert-error"><i class="icon-error"></i> Cannot update timesheet for dates outside the current leave year.</div>';
    } else {
        $total_hours = $hours + ($minutes / 60);
        ...
        if ($owner_data && $owner_data['user_id'] == $user_id && $owner_data['status'] != 'approved') {
            if ($owner_data['entry_date'] == $today_date) {
                ...
                if ($stmt->execute()) {
                    ...
                    $redirect_url = "timesheet.php?from_date=" . urlencode($from_date) . "&to_date=" . urlencode($to_date) . "&year=" . urlencode($year_filter);
                    if (isset($_GET['user_id']) && $_GET['user_id'] != '') {
                        $redirect_url .= "&user_id=" . urlencode($_GET['user_id']);
                    }
                    header("Location: $redirect_url");
...
if (!$is_previous_year && isset($_GET['approve']) && in_array($role, ['pm', 'hr', 'admin'])) {
    ...
    if ($ts_data) {
        // Check if entry is within current year
        if ($ts_data['entry_date'] < $year_start_date || $ts_data['entry_date'] > $year_end_date) {
            $message = '<div class="alert alert-error"><i class="icon-error"></i> Cannot approve timesheet entries from previous years.</div>';
        } else {
...
if (!$is_previous_year && isset($_GET['reject']) && in_array($role, ['pm', 'hr', 'admin'])) {
    ...
    if ($ts_data) {
        // Check if entry is within current year
        if ($ts_data['entry_date'] < $year_start_date || $ts_data['entry_date'] > $year_end_date) {
            $message = '<div class="alert alert-error"><i class="icon-error"></i> Cannot reject timesheet entries from previous years.</div>';
        } else {
...
if (!$is_previous_year && isset($_GET['delete']) && in_array($role, ['pm', 'hr', 'admin'])) {
    ...
    // Check if entry is within current year
    if ($entry_date < $year_start_date || $entry_date > $year_end_date) {
        $message = '<div class="alert alert-error"><i class="icon-error"></i> Cannot delete timesheet entries from previous years.</div>';
    } else if (isHoliday($entry_date) !== false) {
...
// Auto-check for previous day - runs only ONCE per day per session (performance optimization)
$lop_today_key = 'lop_checked_' . date('Y-m-d');
if (in_array($role, ['hr', 'admin', 'pm']) && !isset($_SESSION[$lop_today_key])) {
    $_SESSION[$lop_today_key] = true; // Mark as done for today
    $yesterday = date('Y-m-d', strtotime('-1 day'));
    
    // Only check if yesterday is within current year
    if ($yesterday >= $year_start_date && $yesterday <= $year_end_date) {
        if (date('N', strtotime($yesterday)) == 7) {
            // Sunday - no LOP, no auto timesheet
        }
        else if (isHoliday($yesterday) !== false) {
            // Holiday - no LOP, no auto timesheet
        }
        else {
            $users_result = $conn->query("SELECT id, username, role, status, duty_project_id, duty_task_name, duty_hours, duty_software FROM users");
            $lop_added_count = 0;
            $auto_ts_count = 0;
            
            while ($user = $users_result->fetch_assoc()) {
                // SKIP PM/HR/ADMIN users - they don't get LOP or auto timesheet
                if (in_array($user['role'], ['hr', 'admin', 'pm'])) {
                    continue;
                }
                ...
                if ($ts_result->num_rows == 0) {
                    // ON DUTY: auto-submit timesheet instead of LOP
                    if ($user['status'] === 'on_duty') {
                        if (!hasApprovedLeaveOnDate($conn, $user['id'], $yesterday)) {
                            // Get project name for the duty project
                            $duty_project_id = $user['duty_project_id'] ?? null;
                            $duty_project_name = '';
                            if ($duty_project_id) {
                                $proj_stmt = $conn->prepare("SELECT project_name, project_code FROM projects WHERE id = ?");
                                $proj_stmt->bind_param("i", $duty_project_id);
                                $proj_stmt->execute();
                                $proj_row = $proj_stmt->get_result()->fetch_assoc();
                                $proj_stmt->close();
                                if ($proj_row) {
                                    $duty_project_name = $proj_row['project_name'];
                                    if (!empty($proj_row['project_code'])) {
                                        $duty_project_name .= ' [' . $proj_row['project_code'] . ']';
                                    }
                                }
                            }
                            $duty_hours = floatval($user['duty_hours'] ?? 8);
                            $duty_task = $user['duty_task_name'] ?? 'On Duty';
                            $duty_software = $user['duty_software'] ?? 'Other';
                            $duty_remarks = 'Auto-submitted (On Duty)';
                            $duty_status = 'completed';
                            $submitted_date = $yesterday . ' 23:59:00'; // Mark as submitted at end of day
                            ...
                            $ins->bind_param("isdsisssss", $user['id'], $yesterday, $duty_hours, $duty_software, $duty_project_id, $duty_project_name, $duty_task, $duty_remarks, $duty_status, $submitted_date);
                            if ($ins->execute()) {
                                $auto_ts_count++;
                            }
                            $ins->close();
                        ...
                        // Regular active user - add LOP if no leave
                        if (!hasApprovedLeaveOnDate($conn, $user['id'], $yesterday)) {
            ...
            $msgs = [];
            if ($lop_added_count > 0) $msgs[] = $lop_added_count . ' LOP entries added';
            if ($auto_ts_count > 0) $msgs[] = $auto_ts_count . ' auto-timesheets submitted for On Duty users';
            if (!empty($msgs)) {
                $message = '<div class="alert alert-info"><i class="icon-info"></i> Auto-check for ' . $yesterday . ': ' . implode(', ', $msgs) . ' (PM/HR/Admin excluded).</div>';
            }
        }
    }
}

// Get timesheet entries for selected year
...
// Calculate total hours for the selected year/range
if (in_array($role, ['hr', 'admin', 'pm'])) {
    if (isset($_GET['user_id']) && $_GET['user_id'] != '') {
...
// Get LOP count for the selected year/range
$lop_count = 0;
if (in_array($role, ['hr', 'admin', 'pm'])) {
    if (isset($_GET['user_id']) && $_GET['user_id'] != '') {
...
// Get total submissions for the selected year/range
$submissions_count = 0;
...
if (isset($_GET['export_excel']) && in_array($role, ['hr', 'admin', 'pm'])) {
    ...
    $export_year = isset($_GET['year']) ? sanitize($_GET['year']) : $year_filter;
    
    // Build filename from date range, user, and year
    ...
    if ($export_user_id > 0) {
        ...
        $ts_filename = $from_fmt . '_to_' . $to_fmt . '_' . $user_label . '_' . $export_year . '.xls';
    } else {
        $ts_filename = $from_fmt . '_to_' . $to_fmt . '_all_' . $export_year . '.xls';
    }
    ...
    echo '<div class="summary-row"><span class="summary-label">Leave Year:</span><span class="summary-value">' . $export_year . ' (Mar 16 - Mar 15)</span></div>';
...
<body>
        ...
        <div class="main-content">
            ...
            <div class="year-filter">
                ...
                <div class="year-badge <?php echo $is_previous_year ? 'previous-year-badge' : ''; ?>">
                    ...
                    <?php echo $is_previous_year ? 'Viewing Archived Year' : 'Current Leave Year'; ?>: <?php echo $year_filter; ?>
                </div>
            </div>
            ...
            <div class="archive-note">
                <i class="icon-info"></i> 
                <strong>You are viewing archived timesheet data from leave year <?php echo $year_filter; ?>.</strong> 
                These records are read-only and cannot be modified. Any actions (edit/approve/reject/delete) are disabled.
            </div>
            <?php endif; ?>
            
            <?php echo $message; ?>
            
            <?php if (in_array($role, ['hr', 'admin', 'pm']) && !$is_previous_year): ?>
            ...
            <div class="project-management" style="background: #fed7d7; border-color: #fc8181;">
                ...
                <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                    ...
                    <a href="?check_missing=1&check_date=<?php echo $yesterday_btn; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year=<?php echo $year_filter; ?>" class="btn-check" style="background: #805ad5; color: white; padding: 8px 16px; border-radius: 6px; text-decoration: none;">
                    ...
                    <form method="GET" action="" style="display: flex; align-items: center; gap: 6px;">
                        ...
                        <input type="hidden" name="user_id" value="<?php echo $_GET['user_id']; ?>">
                        <?php endif; ?>
                        <input type="date" name="check_date" value="<?php echo $yesterday_btn; ?>" style="padding: 7px 10px; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 13px;">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div style="display: flex; gap: 10px; align-items: center;">
                        ...
                        <div class="export-container">
                            <a href="?export_excel=1&export_from=<?php echo $from_date; ?>&export_to=<?php echo $to_date; ?><?php echo isset($_GET['user_id']) && $_GET['user_id'] != '' ? '&export_user_id=' . $_GET['user_id'] : ''; ?>&year=<?php echo $year_filter; ?>" class="btn-export" target="_blank">
                ...
                <!-- Edit Timesheet Form -->
                <?php if (!$is_previous_year && $edit_mode && $edit_data): ?>
                <div class="timesheet-form" style="border: 2px solid #4299e1;">
                    ...
                    <form method="POST" action="" id="timesheetForm">
                        ...
                        <div style="display: flex; align-items: center;">
                            ...
                            <a href="?cancel_edit=1&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>&year=<?php echo $year_filter; ?>" class="btn-cancel">
                ...
                <!-- Add Timesheet Form -->
                <?php elseif (!$is_previous_year && !$existing_timesheet && (!in_array($role, ['hr', 'admin', 'pm']) || isset($_GET['user_id']))): 
                ...
                <?php endif; ?>
                <?php elseif (!$is_previous_year && $existing_timesheet && (!in_array($role, ['hr', 'admin', 'pm']) || isset($_GET['user_id'])) && !$edit_mode): ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php if ($timesheets && $timesheets->num_rows > 0): ?>
                                ...
                                $can_delete = !$is_previous_year && in_array($role, ['pm', 'hr', 'admin']) && ($entry['id'] > 0) && ($entry['status'] != 'approved');
                                $can_edit = !$is_previous_year && ($entry['user_id'] == $user_id) && ($entry['id'] > 0) && ($entry['status'] != 'approved') && ($entry['entry_date'] == $today_date);
                                ...
                                ?>">
                                    ...
                                    <td>
                                        <div class="action-buttons" style="display: flex; gap: 5px; flex-wrap: wrap;">
                                            ...
                                            <a href="?edit=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>&year=<?php echo $year_filter; ?>" 
                                               class="btn-small btn-edit">
                                                <i class="icon-edit"></i>
                                            </a>
                                            <?php elseif ($can_edit && ($is_sunday || $is_holiday)): ?>
                                            <span class="btn-small btn-edit-disabled" title="Cannot edit holiday or Sunday entries">
                                                <i class="icon-edit"></i>
                                            </span>
                                            ...
                                            <?php 
                                            // Check if delete is allowed - for PM/HR/Admin only for current year
                                            $can_delete_late = !$is_previous_year && in_array($role, ['pm', 'hr', 'admin']) && $is_late && !$is_holiday;
                                            $can_delete_regular = !$is_previous_year && in_array($role, ['pm', 'hr', 'admin']) && !$is_approved && !$is_holiday;
                                            $can_delete_sunday = !$is_previous_year && in_array($role, ['pm', 'hr', 'admin']) && $is_sunday && !$is_holiday;
                                            
                                            if ($can_delete_late || $can_delete_regular || $can_delete_sunday): 
                                            ?>
                                                <?php if ($is_sunday && $has_sunday_leave): ?>
                                                <div class="delete-options">
                                                    <a href="?delete=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>&year=<?php echo $year_filter; ?>" 
                                                       class="btn-small btn-delete"
                                                       onclick="return confirm('Delete timesheet only? The associated Sunday leave will remain.')">
                                                        <i class="icon-delete"></i> TS Only
                                                    </a>
                                                    <a href="?delete=<?php echo $entry['id']; ?>&delete_sunday_leave=1&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>&year=<?php echo $year_filter; ?>" 
                                            ...
                                            <?php elseif ($is_previous_year): ?>
                                                <span class="btn-small btn-delete action-disabled" style="opacity:0.5;" title="Cannot delete archived entries">
                                                    <i class="icon-delete"></i>
                                                </span>
                                            <?php endif; ?>
                                            
                                            <?php if (!$is_previous_year && in_array($role, ['pm', 'hr', 'admin']) && !$is_approved && $is_late && !$is_holiday): ?>
                                            <a href="?approve=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>&year=<?php echo $year_filter; ?>" 
                                            ...
                                            <a href="?reject=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>&year=<?php echo $year_filter; ?>" 
                                            ...
                                            <?php if (!$is_previous_year && in_array($role, ['pm', 'hr', 'admin']) && !$is_approved && $is_sunday && !$is_holiday): ?>
                                            <a href="?approve=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>&year=<?php echo $year_filter; ?>" 
                                            ...
                                            <?php elseif ($is_previous_year && $is_approved): ?>
                                            <span class="btn-small btn-approve-disabled">
                                                <i class="icon-check"></i> Approved
                                            </span>
                                            <?php endif; ?>
                                ...
                                <tr>
                                    ...
                                    ?>" style="text-align: center; padding: 40px;">
                                        <div class="no-timesheet">
                                            <i class="icon-clock"></i>
                                            <div>No timesheet entries found for <?php echo $from_date; ?><?php echo ($from_date != $to_date) ? ' to ' . $to_date : ''; ?> in leave year <?php echo $year_filter; ?></div>
    ...
    function changeUser(userId) {
        ...
        if (fromDate && toDate) {
            if (userId) {
                window.location.href = `timesheet.php?user_id=${userId}&from_date=${fromDate}&to_date=${toDate}&year=${year}`;
            } else {
                window.location.href = `timesheet.php?from_date=${fromDate}&to_date=${toDate}&year=${year}`;
            }
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        <?php if ($tasks_table_exists && !$is_previous_year): ?>
        toggleTaskInput();
