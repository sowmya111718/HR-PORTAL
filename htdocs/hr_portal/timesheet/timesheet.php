...
// For HR/Admin/PM/COO/ED viewing other users' timesheets
if (isset($_GET['user_id']) && in_array($role, ['hr', 'admin', 'pm', 'coo', 'ed'])) {
...
$is_admin_user = in_array(strtolower($role), ['hr', 'admin', 'pm', 'coo', 'ed', 'mg']) || $user_info['reporting_to'] == 'CEO' || $user_info['position'] == 'CEO';
...
// ============================================
// Optimised: bulk-loads all LOPs, timesheets and approved leaves for the
// 30-day window in just 4 queries total, then uses in-memory lookups.
// KEY RULE: if LOP already exists for a user+date, skip immediately — no more queries.
if (in_array($role, ['hr', 'admin', 'pm', 'coo', 'ed'])) {
    $today           = date('Y-m-d');
    $thirty_days_ago = date('Y-m-d', strtotime('-30 days'));
    $yesterday       = date('Y-m-d', strtotime('-1 day'));

    // ── 1. Fetch all employee users ONCE (exclude management roles) ──────────
    $users_all = [];
    $ur = $conn->query("SELECT id, username, role, status, created_at, duty_project_id, duty_task_name, duty_hours, duty_software FROM users WHERE status != 'inactive' AND role NOT IN ('hr','admin','pm','coo','ed','MG')");
    ...
    if (!empty($users_all)) {
        ...
        // ── 2. Bulk-load existing auto-LOPs (key: "uid|date" => true) ────────
        $existing_lops = [];
        $lr = $conn->query("SELECT user_id, from_date FROM leaves WHERE leave_type='LOP' AND reason LIKE 'Auto-generated LOP%' AND user_id IN ($user_ids_str) AND from_date BETWEEN '$thirty_days_ago' AND '$yesterday'");
        while ($row = $lr->fetch_assoc()) {
            $existing_lops[$row['user_id'] . '|' . $row['from_date']] = true;
        }
        $lr->close();

        // ── 3. Bulk-load existing timesheets (key: "uid|date" => status) ─────
        $existing_ts = [];
        $tr = $conn->query("SELECT user_id, entry_date, status FROM timesheets WHERE user_id IN ($user_ids_str) AND entry_date BETWEEN '$thirty_days_ago' AND '$yesterday'");
        while ($row = $tr->fetch_assoc()) {
            $existing_ts[$row['user_id'] . '|' . $row['entry_date']] = $row['status'];
        }
        $tr->close();

        // ── 4. Bulk-load approved leaves (key: "uid|date" => true) ───────────
        $approved_leaves = [];
        $al = $conn->query("SELECT user_id, from_date, to_date FROM leaves WHERE status='Approved' AND leave_type != 'LOP' AND user_id IN ($user_ids_str) AND from_date <= '$yesterday' AND to_date >= '$thirty_days_ago'");
        ...
        // ── 5. Cache duty project names (one query per unique project) ────────
        $duty_project_names = [];
        ...
        // ── 6. Loop dates × users using only in-memory lookups ───────────────
        $start_date = new DateTime($thirty_days_ago);
        ...
        foreach ($date_range as $date_obj) {
            ...
            // Skip Sundays
            if (date('N', strtotime($check_date)) == 7) continue;

            // Skip holidays
            if (isHoliday($check_date) !== false) continue;
            ...
            foreach ($users_all as $user) {
                ...
                $ts_status  = $existing_ts[$key] ?? null;  // null = no timesheet
                $has_leave  = isset($approved_leaves[$key]);

                // KEY OPTIMISATION: LOP already exists and no approved timesheet → skip, no DB call
                if ($lop_exists && $ts_status !== 'approved') continue;

                if (!$ts_status && !$has_leave) {
                    // No timesheet, no approved leave → need LOP or on-duty auto-timesheet
                    ...
                    if ($user['status'] === 'on_duty') {
                        ...
                        // Regular employee — add LOP only if not already present
                        if (!$lop_exists) {
                    ...
                    // Approved timesheet but LOP still present — remove LOP
                    $uid = intval($user['id']);
                    $conn->query("DELETE FROM leaves WHERE user_id=$uid AND from_date='$check_date' AND leave_type='LOP' AND reason LIKE 'Auto-generated LOP%'");
                    $lop_removed_count++;
                    unset($existing_lops[$key]);
    ...
    // Show message only if changes were made
    if ($lop_added_count > 0 || $auto_ts_count > 0 || $lop_removed_count > 0) {
        $msgs = [];
        if ($lop_added_count   > 0) $msgs[] = $lop_added_count   . ' new LOP entries added';
        if ($auto_ts_count     > 0) $msgs[] = $auto_ts_count     . ' auto-timesheets submitted for On Duty users';
        if ($lop_removed_count > 0) $msgs[] = $lop_removed_count . ' LOP entries removed (approved timesheets found)';
        $message = '<div class="alert alert-info"><i class="icon-info"></i> Auto-check completed: ' . implode(', ', $msgs) . '.</div>';
    }
}
...
// PM/COO/ED Approval action - WITH NOTIFICATION FOR EMPLOYEE (only for late entries)
if (isset($_GET['approve']) && in_array($role, ['pm', 'ed'])) {
    ...
    $message = '<div class="alert alert-error"><i class="icon-error"></i> Admins cannot approve timesheets. Use PM or ED.</div>';
} elseif (isset($_GET['approve']) && ($role == 'hr' || $role == 'coo')) {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> HR/COO cannot approve timesheets. Use PM or ED.</div>';
}

// PM/COO/ED Rejection action - WITH NOTIFICATION FOR EMPLOYEE (only for late entries)
if (isset($_GET['reject']) && in_array($role, ['pm', 'ed'])) {
    ...
    $message = '<div class="alert alert-error"><i class="icon-error"></i> Admins cannot reject timesheets. Use PM or ED.</div>';
} elseif (isset($_GET['reject']) && ($role == 'hr' || $role == 'coo')) {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> HR/COO cannot reject timesheets. Use PM or ED.</div>';
}

// Delete timesheet entry - Only for PM, COO, ED (NOT Admin)
if (isset($_GET['delete']) && in_array($role, ['pm', 'ed'])) {
    ...
    if (isHoliday($entry_date) !== false) {
        ...
        try {
            ...
            // Notify all HR/Admin/PM/COO/ED about the deletion
            $mgmt_users = $conn->query("SELECT id FROM users WHERE role IN ('hr', 'admin', 'pm', 'coo', 'ed') AND id NOT IN ($timesheet_user_id, $user_id)");
    ...
    $message = '<div class="alert alert-error"><i class="icon-error"></i> Admins cannot delete timesheet entries. Use PM or ED.</div>';
} elseif (isset($_GET['delete']) && (strtolower($role) == 'hr' || strtolower($role) == 'coo')) {
    $message = '<div class="alert alert-error"><i class="icon-error"></i> HR/COO cannot delete timesheet entries. Use PM or ED.</div>';
} elseif (isset($_GET['delete']) && !in_array(strtolower($role), ['pm', 'ed', 'admin', 'hr', 'coo'])) {
...
if (isset($_GET['user_id']) && $_GET['user_id'] != '' && in_array($role, ['hr', 'admin', 'pm', 'coo', 'ed'])) {
    foreach ($users as $u) {
...
// ============================================
if (isset($_GET['export_excel']) && in_array($role, ['hr', 'admin', 'pm', 'coo', 'ed'])) {
    ...
    foreach ($all_users as $user) {
        ...
        if (isset($timesheets_data[$user_key])) {
            foreach ($timesheets_data[$user_key] as $date => $entry) {
                ...
                if (date('N', strtotime($date)) == 7) {
                    // Sunday: only count if approved by PM/COO/ED
                    if ($entry['status'] === 'approved') {
    ...
    <div class="app-main">
        ...
        <div class="main-content">
            <h2 class="page-title">
                ...
                <?php echo $page_title; ?>
                <?php if (in_array($role, ['hr', 'admin', 'pm', 'coo', 'ed']) && !isset($_GET['user_id'])): ?>
            ...
            <?php if (in_array($role, ['hr', 'admin', 'pm', 'coo', 'ed'])): ?>
            ...
            <div class="card">
                        ...
                        <form method="GET" action="" id="filterForm" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                            ...
                            <div class="filter-container">
                                <div class="filter-item">
                                    <label><i class="icon-calendar"></i> Leave Year (Mar 16 - Mar 15):</label>
                                    <select name="year_filter" class="form-control" onchange="this.form.submit()">
                                        ...
                                        <?php foreach ($leave_year_options as $year_option): ?>
                                            <option value="<?php echo $year_option['value']; ?>" <?php echo ($year_filter == $year_option['value']) ? 'selected' : ''; ?>>
                ...
                <div style="margin-bottom: 20px; padding: 15px; background: #f7fafc; border-radius: 10px;">
                    <div style="display: flex; gap: 20px; margin-bottom: 10px; flex-wrap: wrap;">
                      ...
                      ?>
                      <div>
                          <strong>Total Hours <?php echo in_array($role, ['hr', 'admin', 'pm', 'coo', 'ed']) ? '(' . $from_date . ' to ' . $to_date . ')' : 'Today'; ?>:</strong> 
                          <span style="font-weight: bold; color: #276749;"><?php echo $total_display; ?></span>
                          <?php if ($working_days_in_range > 0 && in_array($role, ['hr', 'admin', 'pm', 'coo', 'ed'])): ?>
                ...
                <!-- Add Timesheet Form -->
                <?php elseif (!$existing_timesheet && (!in_array(strtolower($role), ['hr', 'admin', 'pm', 'coo', 'ed', 'mg']) || isset($_GET['user_id']))): 
                ...
                <?php endif; ?>
                <?php elseif ($existing_timesheet && (!in_array(strtolower($role), ['hr', 'admin', 'pm', 'coo', 'ed', 'mg']) || isset($_GET['user_id'])) && !$edit_mode): ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php if ($timesheets && $timesheets->num_rows > 0): ?>
                                ...
                                $can_delete = in_array($role, ['pm', 'ed']) && ($entry['id'] > 0) && ($entry['status'] != 'approved');
                                ...
                                ?>">
                                    ...
                                    <td>
                                        <div class="action-buttons" style="display: flex; gap: 5px; flex-wrap: wrap;">
                                            ...
                                            <?php 
                                            $can_delete_late = in_array($role, ['pm', 'ed']) && $is_late && !$is_holiday;
                                            $can_delete_regular = in_array($role, ['pm', 'ed']) && !$is_approved && !$is_holiday;
                                            $can_delete_sunday = in_array($role, ['pm', 'ed']) && $is_sunday && !$is_holiday;
                                            ...
                                            <?php if (in_array($role, ['pm', 'ed']) && !$is_approved && $is_late && !$is_holiday): ?>
                                            ...
                                            <?php if (in_array($role, ['pm', 'ed']) && !$is_approved && $is_sunday && !$is_holiday): ?>
                                            <a href="?approve=<?php echo $entry['id']; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&year_filter=<?php echo $year_filter; ?><?php echo isset($_GET['user_id']) ? '&user_id=' . $_GET['user_id'] : ''; ?>" 
                            ...
                            <?php if ($timesheets->num_rows == 0 && empty($lop_entries)): ?>
                                <tr>
                                    <td colspan="<?php 
                                        $colspan = 10;
                                        if (in_array($role, ['hr', 'admin', 'pm', 'coo', 'ed'])) $colspan += 1;
                                        echo $colspan;
