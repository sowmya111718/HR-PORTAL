...
if ($hol_today_row) {
    ...
    // Check TOMORROW in both tables
    $hol_row = null;
    $hol_check = $conn->prepare("SELECT holiday_name FROM holidays WHERE holiday_date = ?");
    $hol_check->bind_param("s", $tomorrow);
    $hol_check->execute();
    $hol_row = $hol_check->get_result()->fetch_assoc();
    $hol_check->close();

    if (!$hol_row) {
        // Also check wish_holidays for tomorrow
        ...
        if ($wish_check2 && $wish_check2->num_rows > 0) {
            ...
            $wh_stmt2->execute();
            $hol_row = $wh_stmt2->get_result()->fetch_assoc();
            $wh_stmt2->close();
        }
    }

    if ($hol_row) {
        $advance_holiday  = $hol_row['holiday_name'];
        $advance_theme    = matchHolidayTheme($advance_holiday, $holiday_themes);
        $advance_is_today = false;
    }
}
    ...
    <div class="birthday-message">
        <h3>
            <?php if ($is_my_birthday): ?>
                🎉 Happy Birthday to You, <?php echo htmlspecialchars($full_name); ?>! 🎉
            <?php elseif (count($today_birthdays) == 1): ?>
                🎉 Wish Happy Birthday to <?php echo htmlspecialchars($today_birthdays[0]['full_name']); ?>! 🎉
            <?php else: ?>
...
<!-- ── Advance Birthday Wishes Banner ─────────────────────────────── -->
<div class="holiday-advance-banner" style="background: linear-gradient(135deg, #116fbd 0%, #3790d8 50%, #3790d8 100%); color: #fff;">
    <div class="holiday-decor-strip">
        🎂 🎈 🎉 🎁 🎊 🎂 🎈 🎉 🎁 🎊 🎂 🎈 🎉 🎁 🎊 🎂 🎈 🎉 🎁 🎊 🎂 🎈 🎉 🎁 🎊 🎂 🎈 🎉 🎁 🎊
    </div>
    <div class="holiday-emoji-big">🎂</div>
    <div class="holiday-advance-msg">
        <h3>🎈 Advance Birthday Wishes! 🎈</h3>
        <p>
            <?php
            $names = array_column($advance_birthdays, 'full_name');
            if (count($names) === 1) {
                echo 'Tomorrow is <strong>' . htmlspecialchars($names[0]) . '</strong>\'s Birthday! Wishing them a wonderful celebration! 🎊';
            } else {
                $last = array_pop($names);
                echo 'Tomorrow is the Birthday of <strong>' . implode(', ', array_map('htmlspecialchars', $names)) . '</strong> and <strong>' . htmlspecialchars($last) . '</strong>! Wishing them a wonderful celebration! 🎊';
            }
            ?>
        </p>
        <span class="holiday-tag">🎂 <?php echo date('d M Y', strtotime('+1 day')); ?></span>
    </div>
    <div class="holiday-emoji-big">🎂</div>
</div>
<!-- ──────────────────────────────────────────────────────────────────── -->
