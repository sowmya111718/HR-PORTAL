...
// For HR/Admin/PM - Get counts of new submissions
            ...
            <div class="birthday-message">
                <h3>
                    <?php if ($is_my_birthday): ?>
                        🎉 Happy Birthday to You, <?php 
                            $name_parts = explode(' ', $full_name);
                            echo htmlspecialchars(isset($name_parts[1]) ? $name_parts[1] : $name_parts[0]); 
                        ?>! 🎉
                    <?php elseif (count($today_birthdays) == 1): ?>
                        <?php 
                            $birthday_name = $today_birthdays[0]['full_name'];
                            $name_parts = explode(' ', $birthday_name);
                            $display_name = isset($name_parts[1]) ? $name_parts[1] : $name_parts[0];
                        ?>
                        🎉 Wish Happy Birthday to <?php echo htmlspecialchars($display_name); ?>! 🎉
                    <?php else: ?>
                ...
                <div class="birthday-names">
                    <?php foreach ($today_birthdays as $bday_user): ?>
                        <?php 
                            $name_parts = explode(' ', $bday_user['full_name']);
                            $display_name = isset($name_parts[1]) ? $name_parts[1] : $name_parts[0];
                        ?>
