...
function deleteOldTimesheets($conn, $from_date, $to_date) {
    ...
    // 2. Delete any auto-generated LOP for these dates so they don't remain
    $lop_stmt = $conn->prepare("DELETE FROM leaves WHERE from_date BETWEEN ? AND ? AND leave_type = 'LOP' AND reason LIKE 'Auto-generated LOP%'");
    $lop_stmt->bind_param("ss", $from_date, $to_date);
    $lop_stmt->execute();
    $lop_stmt->close();

    // 3. Insert cleanup exemption records for all employees for these dates
    // This prevents auto-LOP from regenerating for cleaned-up dates
    $users = $conn->query("SELECT id FROM users WHERE status != 'inactive' AND role NOT IN ('hr','admin','dm','coo','ed','MG')");
    $ins = $conn->prepare("INSERT IGNORE INTO leaves (user_id, leave_type, from_date, to_date, days, day_type, reason, status) VALUES (?, 'LOP', ?, ?, 0, 'full', 'CLEANUP-EXEMPTION: Data archived by admin', 'Cancelled')");
    while ($u = $users->fetch_assoc()) {
        ...
        while ($d <= $end) {
            ...
            // Only exempt working days (not Sundays)
            if (date('N', strtotime($date_str)) != 7) {
                $ins->bind_param("iss", $u['id'], $date_str, $date_str);
                $ins->execute();
            }
            $d->modify('+1 day');
        }
