...
// Check if we're changing another user's password (HR/Admin/PM only)
$target_user_id = isset($_GET['user_id']) ? intval($_GET['user_id']) : $user_id;

// Only allow HR/Admin/PM to change others' passwords
if ($target_user_id != $user_id && !in_array($role, ['hr', 'admin', 'pm'])) {
...
// Get all users for dropdown (HR/Admin/PM only)
$users = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if (empty($new_password) || empty($confirm_password)) {
        ...
        if ($target_id == $user_id) {
            ...
            // HR/Admin/PM changing someone else's password - no current password check needed
            $result = updateUserPassword($conn, $target_id, $new_password, $user_id, $role, $target_user);
            $message = $result['message'];
        }
    }
...
<body>
    ...
    <div class="app-main">
        ...
        <div class="main-content">
            ...
            <!-- User Selector (for HR/Admin/PM only) -->
            <?php if (in_array($role, ['hr', 'admin', 'pm']) && count($users) > 0): ?>
