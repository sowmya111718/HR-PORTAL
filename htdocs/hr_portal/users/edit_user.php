...
if ($user) {
    $stmt = $conn->prepare("
        ...
        FROM users 
        WHERE role IN ('hr', 'dm', 'admin', 'MG', 'coo', 'ed') 
        AND id != ?
        ORDER BY 
            CASE role 
                WHEN 'admin' THEN 1
...
<body>
                ...
                <form method="POST" action="" onsubmit="return validateForm()">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <?php $current_role = strtolower($user['role']); // strtolower('DM') = 'dm' ?>
                            <select name="role" class="form-control" required <?php echo $user['id'] == $_SESSION['user_id'] ? 'disabled' : ''; ?>>
                                <option value="employee" <?php echo $current_role === 'employee' ? 'selected' : ''; ?>>Employee</option>
                                <option value="hr" <?php echo $current_role === 'hr' ? 'selected' : ''; ?>>HR Manager</option>
                                <option value="coo" <?php echo $current_role === 'coo' ? 'selected' : ''; ?>>COO (Chief Operating Officer)</option>
                                <option value="DM" <?php echo ($current_role === 'dm' || $user['role'] === 'DM') ? 'selected' : ''; ?>>DM (Delivery Manager)</option>
                                <option value="ed" <?php echo $current_role === 'ed' ? 'selected' : ''; ?>>ED (Executive Director)</option>
                            ...
                            <select name="reporting_to" class="form-control" required>
                                ...
                                <?php if ($managers_result && $managers_result->num_rows > 0): ?>
                                    ...
                                    <option value="<?php echo $manager['username']; ?>" <?php echo $user['reporting_to'] === $manager['username'] ? 'selected' : ''; ?>>
                                        ...
                                        elseif ($manager['role'] == 'coo') $role_icon = '🏢 ';
                                        elseif ($manager['role'] == 'dm') $role_icon = '📋 ';
                                        elseif ($manager['role'] == 'ed') $role_icon = '🎯 ';
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label class="form-label">Status
                                <?php if (!in_array($_SESSION['role'], ['dm', 'admin', 'ed'])): ?>
                                <span style="color: #e53e3e; font-size: 11px; margin-left: 5px;">(Only dm/Admin can change)</span>
                                <?php endif; ?>
                            </label>
                            <?php if (in_array($_SESSION['role'], ['dm', 'admin', 'ed'])): ?>
                            ...
                            <small style="color: #a0aec0; display: block; margin-top: 5px;">Only dm or Admin can change user status.</small>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            ?>
                            <input type="date" name="birthday" id="edit_birthday" class="form-control birthday-field" 
    ...
    // Force birthday to stored value only — prevent browser autofill if field was empty
    window.addEventListener('load', function() {
        var storedBirthday = '<?php echo $birthday_value; ?>';
        var bd = document.getElementById('edit_birthday');
        if (bd && storedBirthday === '') {
            bd.value = '';
        }
    });
