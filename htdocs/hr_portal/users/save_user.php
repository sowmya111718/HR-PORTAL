...
// Normalize role - keep DM and MG as uppercase, lowercase others
if ($role !== 'MG' && $role !== 'DM' && strtoupper($role) !== 'DM') {
    $role = strtolower($role);
} elseif (strtolower($role) === 'dm') {
    $role = 'DM'; // Always store Delivery Manager as 'DM'
}
...
// Status - only dm/Admin can change
$status = sanitize($data['status'] ?? 'active');
if (!in_array($_SESSION['role'], ['DM', 'admin', 'ed'])) {
