...
$managers = $conn->query("
    ...
    ORDER BY CASE role WHEN 'dm' THEN 1 WHEN 'ed' THEN 2 WHEN 'hr' THEN 3 WHEN 'coo' THEN 4 ELSE 5 END, full_name
");
...
<body>
                        ...
                        <div class="form-group">
                            ...
                            <select name="reporting_to" id="reporting_to" class="form-control" required>
                                ...
                                <option value="<?php echo htmlspecialchars($manager['username']); ?>" 
                                    ...
                                    <?php echo (strtolower($manager['role']) === 'dm' || $manager['role'] === 'DM' && !$dm_selected) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($manager['full_name']); ?> (<?php echo htmlspecialchars($manager['username']); ?>)
                                    <?php if (strtolower($manager['role']) === 'dm' || $manager['role'] === 'DM') { echo ' — Delivery Manager'; $dm_selected = true; } ?>
                                </option>
                                <?php 
                                    endwhile; 
                                }
                ...
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                    ...
                    <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                        ...
                        <select id="roleFilter" class="form-control" style="width: 150px;" onchange="filterByRole()">
                            ...
                            <option value="dm">DM (Delivery Manager)</option>
                                    ...
                                    <td>
                                        <?php 
                                            ...
                                            if ($role_display == 'MG') { 
                                                ...
                                                $role_display = strtoupper($role_display);
                                            }
                                        ?>
    ...
    window.onload = function() {
        ...
        // Auto-select first dm in Reporting To dropdown for new users
        ...
        if (reportingSelect && reportingSelect.value === '') {
            for (var i = 0; i < reportingSelect.options.length; i++) {
                ...
                // Match options that belong to a dm user (text contains 'dm' label or data-role)
                if (reportingSelect.options[i].getAttribute('data-role') === 'dm') {
