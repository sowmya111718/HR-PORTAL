...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_as_lop_ajax'])) {
    ...
    if ($stmt->execute()) {
        ...
        $notify_users = $conn->query("SELECT id FROM users WHERE role IN ('hr', 'admin', 'pm', 'coo', 'ed')");
...
if (isset($_GET['approve']) && in_array($role, ['pm', 'ed'])) {
    ...
    if ($leave) {
        ...
        try {
            ...
            $mgmt_users = $conn->query("SELECT id FROM users WHERE role IN ('hr', 'admin', 'pm', 'coo', 'ed') AND id NOT IN ($user_id, {$leave['user_id']})");
...
if (isset($_GET['delete_leave']) && in_array($role, ['admin', 'pm', 'ed'])) { // Removed 'hr' from here
    ...
    if ($leave) {
        ...
        try {
            ...
            $mgmt_users = $conn->query("SELECT id FROM users WHERE role IN ('hr', 'admin', 'pm', 'coo', 'ed') AND id != {$user_id}");
            while ($mgmt_user = $mgmt_users->fetch_assoc()) {
                                ...
                                <tr <?php echo $is_lop ? 'class="lop-row"' : ''; ?>>
                                    ...
                                    <td>
                                        <div class="action-buttons" style="display: flex; gap: 5px; flex-wrap: wrap;">
                                            ...
                                            <?php if ($leave['status'] === 'Approved' && in_array($role, ['admin', 'pm', 'ed'])): ?>
