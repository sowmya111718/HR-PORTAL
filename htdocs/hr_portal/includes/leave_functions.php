...
function getHolidayList() {
    // Hardcoded fallback holidays (used only if DB is unavailable)
    return [
        '03-19' => 'UGADI',
...
/**
 * Get holidays from database (full YYYY-MM-DD dates)
...
function getHolidaysFromDB() {
    ...
    if (!isset($conn) || !$conn) {
        // Fallback: convert hardcoded list to full dates for current + next year
        $list = getHolidayList();
        $year = date('Y');
    ...
    $check = $conn->query("SHOW TABLES LIKE 'holidays'");
    if (!$check || $check->num_rows == 0) {
        // Table doesn't exist yet — seed it with defaults then return
        ...
        // Seed default holidays for current and next year
        $list = getHolidayList();
        $year = date('Y');
        foreach ([$year, $year + 1] as $y) {
            foreach ($list as $md => $name) {
                $full_date = $y . '-' . $md;
                $conn->query("INSERT IGNORE INTO holidays (holiday_date, holiday_name) VALUES ('$full_date', '$name')");
            }
        }
    }
...
function isHoliday($date) {
    ...
    return isset($holidays[$date_formatted]) ? $holidays[$date_formatted] : false;
}
...
function getHolidaysForYear($year) {
    $all_holidays = getHolidaysFromDB();
    $year_holidays = [];
