...
// UPDATED: Function to add LOP for missing timesheet - SKIPS PM/HR/ADMIN and SKIPS dates with approved leave
function addLOPForMissingTimesheet($conn, $user_id, $date) {
    ...
    // Check if user is PM/HR/Admin - SKIP LOP FOR THEM
    ...
    if ($user_data && in_array(strtolower($user_data['role']), ['hr', 'admin', 'pm', 'coo', 'ed', 'mg'])) {
        return false; // Skip LOP for PM/HR/Admin/COO/ED/MG users - only employees need to submit timesheets
    }
