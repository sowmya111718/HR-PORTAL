...
// Performance: Session-based caching for expensive queries
// Cache TTL: 5 minutes (300 seconds)

// In-memory cache for this request (avoids duplicate queries in same page load)
$_LEAVE_CACHE = [];

function leaveCache($key, $value = null) {
    global $_LEAVE_CACHE;
    if ($value === null) {
        return isset($_LEAVE_CACHE[$key]) ? $_LEAVE_CACHE[$key] : null;
    }
    $_LEAVE_CACHE[$key] = $value;
    return $value;
}

function leaveCacheSession($key, $value = null, $ttl = 300) {
    $session_key = 'lc_' . $key;
    if ($value === null) {
        // Read from session cache
        if (isset($_SESSION[$session_key]) && isset($_SESSION[$session_key . '_t'])) {
            if ((time() - $_SESSION[$session_key . '_t']) < $ttl) {
                return $_SESSION[$session_key];
            }
            // Expired - clear it
            unset($_SESSION[$session_key], $_SESSION[$session_key . '_t']);
        }
        return null;
    }
    // Write to session cache
    $_SESSION[$session_key] = $value;
    $_SESSION[$session_key . '_t'] = time();
    return $value;
}

function clearLeaveCache($user_id = null) {
    global $_LEAVE_CACHE;
    $_LEAVE_CACHE = [];
    // Clear session cache keys for this user
    foreach (array_keys($_SESSION) as $key) {
        if (strpos($key, 'lc_') === 0) {
            if ($user_id === null || strpos($key, "u{$user_id}_") !== false) {
                unset($_SESSION[$key]);
                unset($_SESSION[$key . '_t']);
            }
        }
    }
}
//
// Sick Leave   : Custom Cycle (Mar 16 – Mar 15 next year) — same as Casual
...
// LOP & Permission : Monthly windows (16th – 15th of next month)
// ============================================
...
/**
 * Get all holidays from DB with caching.
 * Falls back to hardcoded list if DB/table unavailable.
 */
function getHolidaysFromDB() {
    global $conn;
    
    // In-memory cache: isHoliday() is called hundreds of times per page load
    $cache_key = 'all_holidays';
    $cached = leaveCache($cache_key);
    if ($cached !== null) return $cached;
    
    // Session cache: 10 min TTL - holidays rarely change
    $cached = leaveCacheSession($cache_key, null, 600);
    if ($cached !== null) {
        leaveCache($cache_key, $cached);
        return $cached;
    }
    ...
    if (!isset($conn) || !$conn) {
        ...
        $list = getHolidayList();
        $year = date('Y');
        foreach ($list as $md => $name) {
            $holidays[$year . '-' . $md] = $name;
            $holidays[($year+1) . '-' . $md] = $name;
        }
        leaveCache($cache_key, $holidays);
        return $holidays;
    }
    ...
    $cnt = $conn->query("SELECT COUNT(*) as c FROM holidays")->fetch_assoc()['c'];
    if ($cnt == 0) {
        $list = getHolidayList();
        $year = date('Y');
        foreach ([$year, $year+1] as $y) {
            foreach ($list as $md => $name) {
                $conn->query("INSERT IGNORE INTO holidays (holiday_date, holiday_name) VALUES ('{$y}-{$md}', '{$name}')");
            }
        }
    }
    
    $result = $conn->query("SELECT holiday_date, holiday_name FROM holidays ORDER BY holiday_date");
    if ($result) {
    ...
    leaveCache($cache_key, $holidays);
    leaveCacheSession($cache_key, $holidays, 600);
    return $holidays;
}

/**
 * Check if a date is a holiday (DB-backed with caching)
 * @param string $date Date in YYYY-MM-DD format
 * @return bool|string Returns false if not holiday, holiday name if it is
 */
function isHoliday($date) {
    $holidays = getHolidaysFromDB();
    $date_fmt = date('Y-m-d', strtotime($date));
    return isset($holidays[$date_fmt]) ? $holidays[$date_fmt] : false;
}
...
// UPDATED: Function to add LOP for missing timesheet - SKIPS dm/HR/ADMIN and SKIPS dates with approved leave
function addLOPForMissingTimesheet($conn, $user_id, $date) {
    ...
    // Skip if user has approved leave on this date (including newly approved leaves)
    if (hasApprovedLeaveOnDate($conn, $user_id, $date)) {
        return false;
    }
    
    // Check if user is dm/HR/Admin - SKIP LOP FOR THEM
    ...
    if ($user_data && in_array(strtolower($user_data['role']), ['hr', 'admin', 'DM', 'coo', 'ed', 'mg'])) {
        return false; // Skip LOP for dm/HR/Admin/COO/ED/MG users - only employees need to submit timesheets
    }
...
// UPDATED: Function to check if user has approved leave on a date - NOW INCLUDES LOP LEAVES as well
function hasApprovedLeaveOnDate($conn, $user_id, $date) {
    // In-memory cache - this is called in tight loops (one call per user per date)
    $cache_key = "leave_on_{$user_id}_{$date}";
    $cached = leaveCache($cache_key);
    if ($cached !== null) return $cached;
    // First check for approved leaves (Sick, Casual, Other, LOP)
    $check = $conn->prepare("
        ...
        AND to_date >= ? 
        AND status = 'Approved'
    "); // Removed the AND leave_type != 'LOP' condition to include approved LOP
    ...
    if ($leave) {
        leaveCache($cache_key, $leave);
        return $leave; // Return the leave data if found
    }
    
    leaveCache($cache_key, false);
    return false; // No approved leave on this date
}

// UPDATED: Function to remove LOP when timesheet is approved OR when leave is approved
function removeLOPForTimesheet($conn, $user_id, $date) {
    clearLeaveCache($user_id); // Invalidate balance cache
    ...
    if ($affected > 0) {
        error_log("Removed auto-generated LOP for user $user_id on date $date due to timesheet approval");
    }
    
    return $affected > 0;
}

// NEW: Function to remove LOP when leave is approved
function removeLOPForLeave($conn, $user_id, $date) {
    $delete = $conn->prepare("DELETE FROM leaves WHERE user_id = ? AND from_date = ? AND leave_type = 'LOP' AND reason LIKE 'Auto-generated LOP%'");
    $delete->bind_param("is", $user_id, $date);
    $delete->execute();
    $affected = $delete->affected_rows;
    ...
    if ($affected > 0) {
        error_log("Removed auto-generated LOP for user $user_id on date $date due to approved leave");
    }
    
    return $affected > 0;
}

// NEW: Function to check and remove LOP for all dates in a leave range
function removeLOPForLeaveRange($conn, $user_id, $from_date, $to_date) {
    $from = new DateTime($from_date);
    $to = new DateTime($to_date);
    $interval = $from->diff($to);
    $removed_count = 0;
    ...
    for ($i = 0; $i <= $interval->days; $i++) {
        $current_date = date('Y-m-d', strtotime($from_date . " + $i days"));
        if (removeLOPForLeave($conn, $user_id, $current_date)) {
            $removed_count++;
        }
    }
...
function getSundayWorkBonusLeaves($conn, $user_id, $casual_year) {
    $stmt = $conn->prepare("
        ...
        WHERE user_id = ? AND leave_type = 'Casual' AND status = 'Approved'
          ...
          AND (reason LIKE '%SUNDAY WORK BONUS%' OR reason LIKE '%FESTIVAL WORK BONUS%')
    ");
...
/**
 * Add +1 Casual Leave for working on a festival/holiday day.
 * Exact same pattern as addCasualLeaveForSundayWork.
 */
function addCasualLeaveForFestivalWork($conn, $user_id, $festival_date, $approved_by) {
    $casual_year = getCasualLeaveYearForDate($festival_date);
    $year_label  = $casual_year['year_label'];
    $festival_name = isHoliday($festival_date) ?: 'Holiday';

    // Check if bonus already exists for this date
    $check = $conn->prepare("
        SELECT id FROM leaves
        WHERE user_id = ? AND from_date = ?
        AND leave_type = 'Casual' AND status = 'Approved'
        AND reason LIKE '%FESTIVAL WORK BONUS%'
    ");
    $check->bind_param("is", $user_id, $festival_date);
    $check->execute();
    $existing = $check->get_result()->fetch_assoc();
    $check->close();

    if (!$existing) {
        $reason       = 'FESTIVAL WORK BONUS - Extra casual leave granted for working on festival day (' . $festival_name . ')';
        $current_date = date('Y-m-d H:i:s');
        ...
        ");
        $insert->bind_param("isssssis",
            $user_id, $festival_date, $festival_date,
            $reason, $current_date, $current_date, $approved_by, $year_label
        );

        if ($insert->execute()) {
            $insert->close();
            // Clear carry-forward cache so balance recalculates immediately
...
// ============================================================
//  PERMISSION MONTHLY RESET FUNCTIONS (16th - 15th)
// ============================================================

/**
 * Get the monthly window for permissions (16th - 15th of next month)
 * Same as casual window
 */
function getPermissionWindowForDate($date) {
    return getWindowForDate($date);
}

/**
 * Get current permission window
 */
function getCurrentPermissionWindow() {
    return getWindowForDate(date('Y-m-d'));
}

/**
 * Get permission hours used in a specific window (16th - 15th)
 */
function getPermissionHoursUsedInWindow($conn, $user_id, $window_start, $window_end) {
    $stmt = $conn->prepare("
...
/**
 * Get permission LOP hours used in a specific window (16th - 15th)
 */
function getPermissionLOPHoursUsedInWindow($conn, $user_id, $window_start, $window_end) {
    $stmt = $conn->prepare("
        SELECT COALESCE(SUM(duration), 0) as total_hours 
        FROM permissions 
        WHERE user_id = ? 
...
/**
 * Get remaining permission hours for current window
 */
function getRemainingPermissionHours($conn, $user_id) {
    $window = getCurrentPermissionWindow();
    $used_hours = getPermissionHoursUsedInWindow($conn, $user_id, $window['window_start'], $window['window_end']);
    return max(0, 4 - $used_hours);
}

// ============================================================
//  LOP MONTHLY RESET FUNCTIONS (16th - 15th)
// ============================================================

/**
 * Get LOP days used in a specific window (16th - 15th)
 */
function getLOPDaysUsedInWindow($conn, $user_id, $window_start, $window_end) {
    $stmt = $conn->prepare("
    ...
    return floatval($row['total_days']);
}

/**
 * Get current LOP window
 */
function getCurrentLOPWindow() {
    return getWindowForDate(date('Y-m-d'));
}

/**
 * Get LOP window for a specific date
 */
function getLOPWindowForDate($date) {
    return getWindowForDate($date);
}
...
function calculateCasualEntitledDays($join_date, $casual_year, $sunday_bonus = 0) {
    ...
    // Determine first qualifying window using getWindowForDate.
    // Rule: the window the employee joins IN always counts for entitlement.
    // e.g. join Mar 16-Apr 15 (any day) = window month_key 2026-03 = all 12 windows = 12 casual days.
    // e.g. join Apr 16-May 15 = window month_key 2026-04 = 11 windows = 11 casual days.
    $join_window = getWindowForDate($join_date);
    $first_ym    = $join_window['month_key'];
...
function calculateCasualAccruedDays($join_date, $casual_year = null) {
    ...
    if ($join <= $cycle_start) {
        ...
        // Use getWindowForDate so joining on 16th or later still counts that window
        $join_window = getWindowForDate($join_date);
        $first_ym    = $join_window['month_key'];
    }
...
function getCasualLeaveUsedInCycle($conn, $user_id, $casual_year) {
    $stmt = $conn->prepare("
        ...
        WHERE user_id = ? AND leave_type = 'Casual' AND status IN ('Approved','Pending')
          AND from_date BETWEEN ? AND ?
          AND (reason NOT LIKE '%SUNDAY WORK BONUS%' AND reason NOT LIKE '%FESTIVAL WORK BONUS%' OR reason IS NULL)
    ");
    $stmt->bind_param("iss", $user_id, $casual_year['start_date'], $casual_year['end_date']);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
...
function checkAndAutoResetCasualLeaveYear($conn) {
    ...
    try {
        ...
        while ($user = $users->fetch_assoc()) {
            ...
            $pu = $conn->prepare("SELECT COALESCE(SUM(days),0) AS tu FROM leaves WHERE user_id=? AND leave_type='Casual' AND status='Approved' AND from_date BETWEEN ? AND ? AND reason NOT LIKE '%SUNDAY WORK BONUS%' AND reason NOT LIKE '%FESTIVAL WORK BONUS%' AND reason NOT LIKE 'Auto-granted: Festival/Holiday%'");
...
// ============================================================
//  SICK LEAVE YEAR  (Mar 16 – Mar 15) - UPDATED TO MATCH CASUAL CYCLE
// ============================================================

function getCurrentLeaveYear() {
    // Sick leave now follows Mar 16 - Mar 15 cycle (same as casual)
    $today = new DateTime();
    $m = (int)$today->format('n');
    $d = (int)$today->format('j');
    $y = (int)$today->format('Y');

    // Jan 1 - Mar 15: cycle started Mar 16 of PREVIOUS year
    // Mar 16 - Dec 31: cycle starts Mar 16 of THIS year
    if ($m < 3 || ($m == 3 && $d < 16)) {
        $start_year = $y - 1;
        $end_year   = $y;
    } else {
    ...
    return [
        ...
        'start_year'  => $start_year,
        'end_year'    => $end_year,
        'is_new_year' => ($today->format('m-d') == '03-16'),
    ];
}
...
function getLeaveYearForDate($date) {
    // Returns Mar 16 - Mar 15 cycle for the given date
...
function hasSickLeaveInPreviousMonth($conn, $user_id) {
    $dmk = getPreviousMonthKey();
    $dms = $dmk . '-01';
    $dme = date('Y-m-t', strtotime($dms));
    ...
    $stmt->bind_param("iss", $user_id, $dms, $dme);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    return $row['count'] > 0;
...
function getSickLeaveUsedThisMonth($conn, $user_id) {
    ...
    // Do not count sick leave taken before the current leave year start (Mar 16)
    $leave_year = getCurrentLeaveYear();
    if ($ms < $leave_year['start_date']) {
        $ms = $leave_year['start_date'];
    }
...
function getCurrentMonthLOPUsage($conn, $user_id) {
    $window = getCurrentLOPWindow();
    $stmt = $conn->prepare("
        SELECT COALESCE(SUM(days),0) AS lop_days 
        FROM leaves 
        WHERE user_id=? AND leave_type='LOP' AND status IN ('Approved','Pending') 
        AND from_date BETWEEN ? AND ?
    ");
    $stmt->bind_param("iss", $user_id, $window['window_start'], $window['window_end']);
    $stmt->execute();
...
function getUserLeaveBalance($conn, $user_id, $leave_type = null) {
    // Cache key includes leave_type to handle both full and filtered calls
    $cache_key = "balance_u{$user_id}_" . ($leave_type ?? 'all');
    $cached = leaveCache($cache_key);
    if ($cached !== null) return $cached;
    $cached = leaveCacheSession($cache_key, null, 120); // 2 min TTL for balance
    if ($cached !== null) { leaveCache($cache_key, $cached); return $cached; }

    $leave_year        = getCurrentLeaveYear();        // Mar 16 - Mar 15 cycle for Sick leave
    ...
    // Used leaves (sick/LOP on Mar 16 - Mar 15 cycle)
...
function getLeaveYearStatistics($conn) {
    ...
    $casual_year    = getCurrentCasualLeaveYear();
    $current_window = getCurrentCasualWindow();
    $permission_window = getCurrentPermissionWindow();
    $lop_window = getCurrentLOPWindow();
    ...
    return [
        ...
        'days_until_reset'         => $today->diff($next_sick_reset)->days, // sick year reset (Mar 15)
        'reset_date'               => $next_sick_reset->format('Y-m-d'),
        'is_reset_period'          => ($today->format('m-d') == '03-16'),
        // Casual cycle
        'casual_leave_year'        => $casual_year['year_label'],
        'casual_year_start'        => $casual_year['start_date'],
        'casual_year_end'          => $casual_year['end_date'],
        ...
        // Permission window
        'permission_window_start'  => $permission_window['window_start'],
        'permission_window_end'    => $permission_window['window_end'],
        // LOP window
        'lop_window_start'         => $lop_window['window_start'],
        'lop_window_end'           => $lop_window['window_end'],
...
/**
 * Reset sick/LOP balances for new calendar year (Mar 16).
...
/**
 * Call on every login.
 * Handles BOTH: sick year reset (Mar 16) + casual cycle reset (Mar 16).
 */
...
/**
 * Get next sick leave reset date (next Mar 16)
 */
function getNextSickResetDate() {
    ...
    // Sick leave resets on Mar 16
    if ($month_now == 3 && $day_now == 16) {
        // Today is reset day
        return $year_now . '-03-16';
    } else if ($month_now < 3 || ($month_now == 3 && $day_now < 16)) {
        // Next reset is Mar 16 of this year
        return $year_now . '-03-16';
    } else {
        // Next reset is Mar 16 of next year
        return ($year_now + 1) . '-03-16';
    }
}
...
/**
 * Get all leave years available in the system (for filtering)
 */
function getAllLeaveYears($conn) {
    $years = [];
    
    // Get from leaves table
    $result = $conn->query("SELECT DISTINCT leave_year FROM leaves WHERE leave_year IS NOT NULL AND leave_year != '' ORDER BY leave_year DESC");
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            $years[] = $row['leave_year'];
        }
    }
    
    // Add current and previous if not present
    $current = getCurrentLeaveYear()['year_label'];
    $previous = getPreviousLeaveYear()['year_label'];
    
    if (!in_array($current, $years)) {
        $years[] = $current;
    }
    if (!in_array($previous, $years)) {
        $years[] = $previous;
    }
...
/**
 * Get all casual leave years available in the system (for filtering)
 */
function getAllCasualLeaveYears($conn) {
    $years = [];
    ...
    // Add current and previous if not present
    $current = getCurrentCasualLeaveYear()['year_label'];
    $previous = getPreviousCasualLeaveYear()['year_label'];
...
/**
 * Convert year label (e.g., "2025-2026") to a proper date for the start of that cycle
 */
function getStartDateFromYearLabel($year_label) {
    // Extract the first year from the label
    $parts = explode('-', $year_label);
    if (count($parts) >= 2) {
        $start_year = $parts[0];
        return $start_year . '-03-16';
    }
