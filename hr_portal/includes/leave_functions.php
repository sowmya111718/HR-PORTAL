...
/**
 * Get month key from date based on 16-15 cycle
 */
function getMonthKeyFromDate($date) {
    ...
    if ($day >= 16) {
        // Date is in first half of leave month (belongs to current month)
        return sprintf("%04d-%02d", $year, $month);
    } else {
        // Date is in second half of leave month (belongs to previous month)
        if ($month == 1) {
...
/**
 * Get user's casual leave carried forward from previous month
 */
function getCasualLeaveCarryForward($conn, $user_id, $month_key) {
    $carry_forward_table = 'casual_leave_carryforward';
    
    // Check if table exists
    $table_check = $conn->query("SHOW TABLES LIKE '$carry_forward_table'");
    if ($table_check->num_rows == 0) {
        return 0;
    }
    
    $stmt = $conn->prepare("
        SELECT carry_forward_days
        FROM $carry_forward_table
    ...
    if ($row = $result->fetch_assoc()) {
        $carry = floatval($row['carry_forward_days']);
        $stmt->close();
        return $carry;
    }
...
/**
 * Save casual leave carry forward for next month
 */
function saveCasualLeaveCarryForward($conn, $user_id, $month_key, $carry_days) {
    ...
    $conn->query("
        CREATE TABLE IF NOT EXISTS $carry_forward_table (
    ...
    // Insert or update
    $stmt = $conn->prepare("
        INSERT INTO $carry_forward_table (user_id, month_key, carry_forward_days)
        VALUES (?, ?, ?)
...
/**
 * Initialize carryforward for a user
 */
function initializeMonthlyCarryForward($conn, $user_id) {
    $current_month_key = getCurrentMonthKey();
    $prev_month_key = getPreviousMonthKey();
    
    // Check if user already has carryforward for current month
    $existing = getCasualLeaveCarryForward($conn, $user_id, $current_month_key);
    
    // Get total casual days used in the entire leave year so far
    $leave_year = getCurrentLeaveYear();
    $total_used_year = 0;
    ...
    $used_stmt = $conn->prepare("
        ...
        AND from_date >= ?
    ");
    $used_stmt->bind_param("is", $user_id, $leave_year['start_date']);
    $used_stmt->execute();
    $used_result = $used_stmt->get_result();
    $used_row = $used_result->fetch_assoc();
    $total_used_year = floatval($used_row['total_used']);
    $used_stmt->close();
    
    // Calculate months passed since March 16
    $start_date = new DateTime($leave_year['start_date']); // March 16
    $today = new DateTime();
    
    // If today is before March 16 of current year, adjust
    if ($today < $start_date) {
        $months_passed = 0;
    } else {
        ...
        // If current day is before 16th, we haven't completed the current month
        if ($today->format('d') < 16) {
            $months_passed = max(0, $months_passed - 1);
        }
    }
    
    // Total entitled = months passed + 1 (for current month if after 16th)
    $total_entitled = $months_passed + 1;
    
    // Remaining = total entitled - total used
    $remaining = max(0, $total_entitled - $total_used_year);
    
    // Save to current month as carryforward
...
function calculateCasualLeaveBalance($conn, $user_id, $month_key = null) {
    ...
    // Initialize carryforward for this month
    $carry_forward = initializeMonthlyCarryForward($conn, $user_id);
    
    // Get used days this month
    $used_this_month = getCasualLeaveUsedThisMonth($conn, $user_id, $month_key);
    
    // Total available = carry_forward (which already includes all months)
    $total_available = $carry_forward + $used_this_month;
    ...
    return [
        'month_key' => $month_key,
        'total_entitled' => $total_available,
...
function getUserLeaveBalance($conn, $user_id, $leave_type = null) {
    ...
    // Calculate Casual leave balance (accumulated)
    ...
    // LOP days taken
    $lop_used = $used['LOP'];
    $casual_used_this_month = $casual_balance['used_this_month'];
    ...
    return [
        ...
        'casual_total_entitled' => $casual_total_entitled,
 ...
 */
function checkLeaveBalance($conn, $user_id, $leave_type, $days) {
    $balance = getUserLeaveBalance($conn, $user_id, $leave_type);
    
    if ($leave_type === 'LOP') {
        // LOP always has balance (unlimited)
        return [
            'has_balance' => true,
            'available' => 999,
    ...
    if ($leave_type === 'Casual') {
        ...
        $available = $casual_balance['remaining'];
        $total_entitled = $casual_balance['total_entitled'];
        
        if ($days <= $available) {
            return [
                'has_balance' => true,
                'available' => $available,
                'message' => "Sufficient balance. You have {$available} casual leave day(s) available (total entitled: {$total_entitled})",
                'monthly_limit' => true,
                'month_key' => $casual_balance['month_key']
            ];
        } else {
            // If they request more than available, EXCESS becomes LOP
            $casual_days = $available;
    ...
    if ($leave_type === 'Sick') {
        $sick_availability = $balance['sick_availability'];
        
        if (!$sick_availability['can_take']) {
            return [
                'has_balance' => false,
                'available' => 0,
                'message' => $sick_availability['reason'],
                'suggest_lop' => true
            ];
        }
        
        $available = $balance['remaining']['Sick'];
        $max_this_month = $sick_availability['max_days'] ?? 1;
        
        if ($days <= $available && $days <= $max_this_month) {
            return [
                'has_balance' => true,
                'available' => $available,
                'max_this_month' => $max_this_month,
                'message' => "Sufficient balance. Available: {$available} days (Max {$max_this_month} this month)"
            ];
        } else {
            $excess_days = $days - min($available, $max_this_month);
            return [
                'has_balance' => false,
                'available' => $available,
                'max_this_month' => $max_this_month,
                'message' => "Insufficient balance. You have {$available} sick days remaining, but only {$max_this_month} can be taken this month.",
                'suggest_lop' => true,
    ...
    if ($days <= $available) {
        return [
            'has_balance' => true,
            'available' => $available,
            'message' => "Sufficient balance. Available: {$available} days"
        ];
    } else {
        return [
            'has_balance' => false,
            'available' => $available,
            'message' => "Insufficient balance. Requested: {$days} days, Available: {$available} days"
        ];
    }
}
...
function applyLeaveWithAutoLOP($conn, $user_id, $leave_type, $from_date, $to_date, $days, $day_type, $reason) {
    ...
    if ($leave_type === 'Casual') {
        // Get current casual balance
        $casual_balance = calculateCasualLeaveBalance($conn, $user_id, $current_month_key);
        $available_casual = $casual_balance['remaining'];
        ...
        try {
            ...
            // Recalculate remaining after using casual days
            $new_remaining = $available_casual - $casual_days;
            
            // Save the remaining days
...
/**
 * NEW FUNCTION: Calculate working days between two dates
 * Working days: Monday to Saturday (1-6), Sunday is off (7)
 */
function calculateWorkingDays($from_date, $to_date, $day_type) {
    ...
    while ($current_date <= $to) {
        ...
        // Count Monday to Saturday (1-6) as working days
        // Sunday (7) is not counted
    ...
    // For half day leaves, each working day counts as 0.5
    if ($day_type === 'half') {
...
function resetLeaveBalancesForNewYear($conn) {
    ...
    try {
        ...
        // Create casual leave carryforward table if not exists
        $conn->query("
