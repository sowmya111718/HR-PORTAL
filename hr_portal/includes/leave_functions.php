 ...
 *
 * Rules:
 *  - Joined on/before the cycle start (Mar 16) → 12 days.
 *  - Joined during the cycle:
 *      Join day ≤ 15 → their join MONTH window counts as first.
 *      Join day > 15 → NEXT month's window is their first.
 *  - Joined after cycle end → 0 days.
 *
 * The "join month window" is the window whose accrual_date month == join month.
 * (Because accrual happens on the 16th, if they join on or before the 15th
 *  they are present BEFORE the accrual on the 16th and qualify for that window.)
 */
function calculateCasualEntitledDays($join_date, $casual_year) {
    ...
    if ($join <= $cycle_start) return 12;

    // Joined after cycle end → 0
    if ($join > $cycle_end) return 0;
 ...
 *   used_in_cycle   = total casual days used in this cycle
 *   remaining       = max(0, accrued_to_date - used_in_cycle)
 *
 * Unused days from past windows automatically carry forward because
 * we always compare TOTAL accrued vs TOTAL used (not window-by-window).
 *
...
function calculateCasualLeaveBalance($conn, $user_id, $month_key = null) {
    ...
    $casual_year    = getCurrentCasualLeaveYear();
    $total_entitled = $join_date ? calculateCasualEntitledDays($join_date, $casual_year) : 12;
    $accrued        = $join_date
        ? calculateCasualAccruedDays($join_date, $casual_year)
        : calculateCasualAccruedDays($casual_year['start_date'], $casual_year);
    ...
    $remaining       = max(0, $accrued - $used_cycle);
...
// ============================================================
//  SICK LEAVE YEAR  (Jan 1 – Dec 31)  — UNCHANGED LOGIC
// ============================================================
...
function getPreviousLeaveYear() {
    ...
    $p = $c['start_year'] - 1;
    return ['start_date' => "{$p}-01-01", 'end_date' => "{$p}-12-31", 'year_label' => "{$p}", 'start_year' => $p, 'end_year' => $p];
}

function getLeaveYearForDate($date) {
    $y = (int)(new DateTime($date))->format('Y');
    return ['start_date' => "{$y}-01-01", 'end_date' => "{$y}-12-31", 'year_label' => "{$y}", 'start_year' => $y, 'end_year' => $y];
}
...
function getUserLeaveBalance($conn, $user_id, $leave_type = null) {
    ...
    if ($join_date) {
        ...
        // Casual (Mar 16–Mar 15 cycle)
        $prorated_entitlement['Casual'] = calculateCasualEntitledDays($join_date, $casual_year);
    }
    ...
    // Casual used: override with cycle-based figure
    $used['Casual'] = getCasualLeaveUsedInCycle($conn, $user_id, $casual_year);
    ...
    $remaining = [
        ...
        'Casual' => $casual_balance['remaining'],
        'LOP'    => $used['LOP'],
    ];
...
function initializeEmployeeLeaveBalance($conn, $user_id, $join_date) {
    ...
    $casual_entitled       = calculateCasualEntitledDays($join_date, $casual_year);
