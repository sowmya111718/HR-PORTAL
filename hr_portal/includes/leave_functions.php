...
/**
 * FIXED: Apply leave with auto LOP conversion - CORRECT DATE SPLITTING
 */
function applyLeaveWithAutoLOP($conn, $user_id, $leave_type, $from_date, $to_date, $days, $day_type, $reason) {
    ...
    if ($leave_type === 'Casual') {
        ...
        try {
            ...
            // If there are casual days available this month
            if ($remaining_this_month > 0) {
                $casual_days = min($remaining_this_month, $days);
                
                // Calculate the end date for casual portion
                $start = new DateTime($from_date);
                $casual_end = clone $start;
                
                // If casual_days > 1, we need to calculate the end date
                // Example: 3 days leave, 1 casual day available -> casual from_date to from_date
                // Example: 3 days leave, 2 casual days available -> casual from_date to from_date+1
                if ($casual_days > 1) {
                    $days_to_add = $casual_days - 1;
                    $casual_end->modify('+' . $days_to_add . ' days');
                }
                $casual_end_date = $casual_end->format('Y-m-d');
                
                $casual_reason = $reason;
                if ($casual_days < $days) {
                    $casual_reason .= " (First " . $casual_days . " day(s) - Casual Leave)";
                }
                ...
                $casual_stmt = $conn->prepare("
                    ...
                    VALUES (?, 'Casual', ?, ?, ?, ?, ?, ?)
                ");
                $casual_stmt->bind_param("issdsss", $user_id, $from_date, $casual_end_date, $casual_days, $day_type, $casual_reason, $year_label);
                $casual_stmt->execute();
                $casual_stmt->close();
            ...
            if ($lop_days > 0) {
                // Calculate the start date for LOP portion
                $lop_start = new DateTime($from_date);
                
                // If there were casual days, LOP starts after casual days
                if ($remaining_this_month > 0) {
                    $casual_days_used = min($remaining_this_month, $days);
                    $lop_start->modify('+' . $casual_days_used . ' days');
                }
                $lop_start_date = $lop_start->format('Y-m-d');
                
                $lop_reason = $reason;
                if ($remaining_this_month > 0) {
                    $lop_reason .= " (Remaining " . $lop_days . " day(s) - Loss of Pay)";
                } else {
                    $lop_reason .= " (Excess leave - Loss of Pay)";
                }
                ...
                $lop_stmt = $conn->prepare("
                    ...
                    VALUES (?, 'LOP', ?, ?, ?, ?, ?, ?)
                ");
                $lop_stmt->bind_param("issdsss", $user_id, $lop_start_date, $to_date, $lop_days, $day_type, $lop_reason, $year_label);
                $lop_stmt->execute();
                $lop_stmt->close();
            ...
            if ($result['lop_days'] > 0) {
                if ($result['casual_days'] > 0) {
                    $result['message'] = "Applied: {$result['casual_days']} casual ( {$from_date} to " . ($casual_end_date ?? $from_date) . " ) + {$result['lop_days']} LOP ( {$lop_start_date} to {$to_date} )";
                } else {
                    $result['message'] = "Applied: {$result['lop_days']} LOP ( {$from_date} to {$to_date} )";
                }
            } else {
                $result['message'] = "Applied: {$result['casual_days']} casual ( {$from_date} to " . ($casual_end_date ?? $from_date) . " )";
            }
        ...
        $stmt = $conn->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param("sssdsss", $user_id, $leave_type, $from_date, $to_date, $days, $day_type, $reason, $year_label);
