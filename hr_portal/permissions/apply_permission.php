...
// Optional: Don't allow past dates (uncomment if needed)
// $today = date('Y-m-d');
// if ($formatted_date < $today) {
//     jsonResponse(false, 'Cannot apply permission for past dates');
// }
...
// Insert permission request with current timestamp
$current_time = date('Y-m-d H:i:s');
$stmt = $conn->prepare("
    INSERT INTO permissions (user_id, permission_date, duration, reason, applied_date)
    VALUES (?, ?, ?, ?, ?)
");
$stmt->bind_param("isdss", $user_id, $formatted_date, $duration, $reason, $current_time);

if ($stmt->execute()) {
    $permission_id = $stmt->insert_id;
    jsonResponse(true, 'Permission request submitted successfully', [
