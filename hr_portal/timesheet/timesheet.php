...
if (isset($_GET['edit'])) {
    ...
    // Check if user has permission to edit (only their own entry and not approved)
    if ($edit_entry && $edit_entry['user_id'] == $user_id && $edit_entry['status'] != 'approved') {
        ...
        $message = '<div class="alert alert-error">You can only edit your own pending timesheet entries.</div>';
    }
}

// NEW: Update timesheet entry after edit
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_timesheet'])) {
    ...
    // Verify the entry belongs to the current user and is not approved
    $check_owner = $conn->prepare("SELECT user_id, status FROM timesheets WHERE id = ?");
    ...
    if ($owner_data && $owner_data['user_id'] == $user_id && $owner_data['status'] != 'approved') {
        ...
        if ($tasks_table_exists && $task_id > 0) {
            $stmt = $conn->prepare("
                UPDATE timesheets 
                SET entry_date = ?, hours = ?, software = ?, project_id = ?, task_id = ?, project_name = ?, task_name = ?, remarks = ?, status = ?
                WHERE id = ?
            ");
            $stmt->bind_param("sdssissssi", $date, $total_hours, $software, $project_id, $task_id, $project_name, $task_name, $remarks, $status, $edit_id);
        } else {
            $stmt = $conn->prepare("
                UPDATE timesheets 
                SET entry_date = ?, hours = ?, software = ?, project_id = ?, project_name = ?, task_name = ?, remarks = ?, status = ?
                WHERE id = ?
            ");
            $stmt->bind_param("sdsissssi", $date, $total_hours, $software, $project_id, $project_name, $task_name, $remarks, $status, $edit_id);
        }
        
        if ($stmt->execute()) {
            $message = '<div class="alert alert-success">Timesheet entry updated successfully!</div>';
            ...
            $message = '<div class="alert alert-error">Error updating timesheet entry: ' . $stmt->error . '</div>';
        }
        $stmt->close();
    } else {
        $message = '<div class="alert alert-error">You can only edit your own pending timesheet entries.</div>';
...
if (isset($_GET['export_excel']) && in_array($role, ['hr', 'admin', 'pm'])) {
    ...
    echo '<p><strong>Period:</strong> ' . date('d-m-Y', strtotime($export_from)) . ' to ' . date('d-m-Y', strtotime($export_to)) . '</p>';
    ...
    // Get all LOP entries for the period (auto-generated from missing timesheets)
    ...
    echo '<tr><td>Date Range (days)</td><td>' . $total_dates . '</td></tr>';
    ...
    echo '<h3>Daily Timesheet Status</h3>';
    echo '<p><span style="background: #d4edda; padding: 2px 8px;">■ Submitted</span> ';
    echo '<span style="background: #fff3cd; padding: 2px 8px;">■ Not Submitted (Auto LOP)</span></p>';
    ...
    foreach ($all_users as $user) {
        ...
        echo '<td style="font-weight: bold; text-align: center;">' . $submission_count . '/' . count($dates) . '</td>';
        echo '</tr>';
    }
    ...
    if ($total_lop_entries > 0) {
        echo '<h3>Auto-generated LOP Entries (Missing Timesheets)</h3>';
        echo '<table border="1" cellpadding="5">';
        echo '<tr class="header">';
        echo '<th>Date</th>';
        echo '<th>Employee</th>';
        ...
        <div class="main-content">
            <h2 class="page-title">
                ...
                <span class="edit-mode-badge">
                    <i class="fas fa-edit"></i> Editing Timesheet Entry
                </span>
                <?php endif; ?>
            </h2>
            ...
            <div class="card">
                ...
                <div class="timesheet-form" style="border: 2px solid #4299e1;">
                    <h4 style="margin-bottom: 20px; color: #4299e1;">
                        <i class="fas fa-edit" style="color: #4299e1;"></i> 
                    ...
                    <form method="POST" action="" id="timesheetForm">
                        ...
                        <div style="display: flex; align-items: center;">
                            <button type="submit" name="update_timesheet" class="btn-submit">
                                <i class="fas fa-save"></i> Update Timesheet Entry
                            </button>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php if ($timesheets && $timesheets->num_rows > 0): ?>
                                ...
                                $can_edit = ($entry['user_id'] == $user_id) && ($entry['id'] > 0) && ($entry['status'] != 'approved');
