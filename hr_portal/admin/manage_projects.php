...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_project'])) {
    ...
    if ($stmt->execute()) {
        ...
        $message = '<div class="alert alert-error">Error adding project: ' . $stmt->error . '</div>';
    }
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_project'])) {
    ...
    if ($stmt->execute()) {
        ...
        $message = '<div class="alert alert-error">Error updating project: ' . $stmt->error . '</div>';
    }
    $stmt->close();
}
...
if (isset($_GET['delete'])) {
    ...
    if ($check_row['count'] > 0) {
        ...
        if ($stmt->execute()) {
            $message = '<div class="alert alert-warning">Project is being used in timesheets. It has been marked as inactive instead of deleted.</div>';
        }
    } else {
        ...
        if ($stmt->execute()) {
            ...
            $message = '<div class="alert alert-error">Error deleting project: ' . $stmt->error . '</div>';
        }
    }
    $stmt->close();
}
        ...
        <div class="main-content">
            ...
            <div class="project-form">
                <h3 style="margin-bottom: 20px; color: #4a5568;">
                    <i class="fas <?php echo $edit_project ? 'fa-edit' : 'fa-plus-circle'; ?>"></i>
                ...
                <form method="POST" action="">
                    ...
                    <div style="display: flex; gap: 10px;">
                        <button type="submit" name="<?php echo $edit_project ? 'edit_project' : 'add_project'; ?>" class="btn">
                            <i class="fas <?php echo $edit_project ? 'fa-save' : 'fa-plus'; ?>"></i>
                ...
                <div class="card-header">
                    ...
                    <div>
                        <span style="color: #718096; margin-right: 10px;">
                            <i class="fas fa-info-circle"></i> Click Delete to remove or mark inactive
                        </span>
                    </div>
                </div>
