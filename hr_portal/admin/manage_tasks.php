...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_task'])) {
    if (!$tasks_table_exists) {
        $message = '<div class="alert alert-error">Tasks table does not exist. Please run the database setup first.</div>';
    } else {
        ...
        if ($stmt->execute()) {
            ...
            $message = '<div class="alert alert-error">Error adding task: ' . $stmt->error . '</div>';
        }
        $stmt->close();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_task'])) {
    ...
    if ($stmt->execute()) {
        ...
        $message = '<div class="alert alert-error">Error updating task: ' . $stmt->error . '</div>';
    }
    $stmt->close();
}
...
if (isset($_GET['delete'])) {
    ...
    if ($check_row['count'] > 0) {
        ...
        if ($stmt->execute()) {
            $message = '<div class="alert alert-warning">Task is being used in timesheets. It has been marked as inactive instead of deleted.</div>';
        }
    } else {
        ...
        if ($stmt->execute()) {
            ...
            $message = '<div class="alert alert-error">Error deleting task: ' . $stmt->error . '</div>';
        }
    }
    $stmt->close();
}
...
<body>
    ...
    <div class="app-main">
        ...
        <div class="main-content">
            <h2 class="page-title">
                <i class="fas fa-check-square"></i> Manage Tasks
            ...
            <div class="db-warning">
                <h3 style="margin-bottom: 10px;"><i class="fas fa-database"></i> Database Setup Required</h3>
...
ADD COLUMN task_name VARCHAR(255) NULL AFTER task_id;
                ...
                <p style="margin-top: 10px;">
                    <a href="../admin/manage_projects.php" class="btn" style="background: #4299e1;">First, Add Projects</a>
                </p>
            </div>
            <?php else: ?>
            ...
            <div class="task-form">
                <h3 style="margin-bottom: 20px; color: #4a5568;">
                    <i class="fas <?php echo $edit_task ? 'fa-edit' : 'fa-plus-circle'; ?>"></i>
                ...
                <form method="POST" action="">
                    ...
                    <div style="display: flex; gap: 10px;">
                        <button type="submit" name="<?php echo $edit_task ? 'edit_task' : 'add_task'; ?>" class="btn">
                            <i class="fas <?php echo $edit_task ? 'fa-save' : 'fa-plus'; ?>"></i>
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-list"></i> All Tasks</h3>
                    <div>
