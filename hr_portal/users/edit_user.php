...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_user'])) {
    ...
    if ($user_id == $_SESSION['user_id'] && $role !== $_SESSION['role']) {
        $message = '<div class="alert alert-error">You cannot change your own role</div>';
    } else {
        $update_stmt = null;
        ...
        if ($update_stmt) {
            if ($update_stmt->execute()) {
                ...
                $message = '<div class="alert alert-error">Error updating user: ' . $update_stmt->error . '</div>';
            }
            $update_stmt->close();
        } else {
            $message = '<div class="alert alert-error">Error preparing statement</div>';
        }
    }
}
?>
        ...
        <div class="main-content">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-user-edit"></i> Edit User: <?php echo $user['full_name']; ?></h3>
                </div>
                            ...
                            <select name="reporting_to" class="form-control" required>
                                ...
                                <?php if ($managers_result && $managers_result->num_rows > 0): ?>
                                    ...
                                    <option value="<?php echo $manager['username']; ?>" <?php echo $user['reporting_to'] === $manager['username'] ? 'selected' : ''; ?>>
                                        <?php echo $manager['full_name']; ?> (<?php echo $manager['username']; ?>)
                                    </option>
                                    <?php endwhile; ?>
                                <?php endif; ?>
                            </select>
            ...
            <?php else: ?>
                <div class="card">
                    <div style="text-align: center; padding: 40px;">
                        <i class="fas fa-user-slash" style="font-size: 48px; color: #cbd5e0; margin-bottom: 15px;"></i>
