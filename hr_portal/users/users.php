...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_user'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            ...
            $message = '<div class="alert alert-error">Error creating user: ' . $stmt->error . '</div>';
        }
    }
    $stmt->close();
}
    ...
    if ($user_id == $_SESSION['user_id']) {
        ...
        try {
            ...
            if ($stmt5->affected_rows > 0) {
                $conn->commit();
                $message = '<div class="alert alert-success">User and all associated records deleted successfully</div>';
            } else {
                $conn->rollback();
            ...
            $conn->rollback();
            $message = '<div class="alert alert-error">Error deleting user: ' . $e->getMessage() . '</div>';
        }
    }
}
...
$managers = $conn->query("
    ...
    FROM users 
    WHERE role IN ('hr', 'manager', 'pm', 'admin', 'MG') 
    ORDER BY full_name
");
...
<head>
    ...
    <style>
        ...
        .role-manager, .role-pm, .role-PM, .role-MG {
            background: #fef3c7;
            color: #92400e;
        }
        .role-employee {
            ...
            <div class="card">
                ...
                <form method="POST" action="" id="addUserForm">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <select name="reporting_to" id="reporting_to" class="form-control" required>
                                ...
                                ?>
                                <option value="Management">Management (Executive Level)</option>
                            </select>
                        </div>
                        <div class="form-group">
                    ...
                    <div id="leavePreview" style="display: none; margin-bottom: 20px; padding: 15px; background: #f0fff4; border: 1px solid #9ae6b4; border-radius: 8px;">
                        <h4 style="color: #276749; margin-bottom: 10px;"><i class="fas fa-calculator"></i> Prorated Leave Balance Preview</h4>
                                    ...
                                    <td>
                                        <?php 
                                            if ($user['reporting_manager']) {
                                                ...
                                                echo 'Management (Executive Level)';
                                            } else {
                                                echo '-';
                                            }
                                        ?>
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('previewYear').innerHTML = 
                            '<i class="fas fa-calendar-alt"></i> Leave Year: ' + data.leave_year;
