...
function getLOPThisMonthCount($conn, $user_id) {
    $stmt = $conn->prepare("
...
function getLOPPreviousMonthCount($conn, $user_id) {
    $stmt = $conn->prepare("
        SELECT COALESCE(SUM(days), 0) as total 
        FROM leaves 
        WHERE user_id = ? AND leave_type = 'LOP' AND status = 'Approved'
...
$lop_this_month_days = getLOPThisMonthCount($conn, $user_id);
$lop_prev_month_days = getLOPPreviousMonthCount($conn, $user_id);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        body { background: linear-gradient(135deg,  #006400 0%, #2c9218 100%); min-height: 100vh; padding: 20px; }
        ...
        /* NEW WELCOME SLIDE FORMAT - Split Screen with Animated Logo */
        .welcome-overlay {
            position: fixed;
            top: 0;
            left: 0;
...
<?php if ($show_welcome): ?>
<!-- NEW WELCOME SLIDE - Split Screen with Animated Logo -->
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card lop-card">
                ...
                <div class="stat-value"><?php echo $lop_this_month_days; ?></div>
                <div class="stat-label">LOP (Leaves) — This Month</div>
                <div class="stat-sub">
                    <div>Yearly total: <?php echo $lop_total; ?> days</div>
                    <div>Previous month: <?php echo $lop_prev_month_days; ?> days</div>
                    <div>From approved leave applications</div>
                </div>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr class="lop-row">
                            ...
                            <td>N/A</td>
                            <td><?php echo $lop_this_month_days; ?> days</td>
                        </tr>
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title"><i class="icon-lop"></i> Recent LOP Leaves (Approved)</h3>
                <a href="leaves/leaves.php" style="background:#4299e1; color:white; padding:6px 14px; border-radius:6px; text-decoration:none; font-size:13px;"><i class="icon-view"></i> View All</a>
            </div>
            <div class="table-container">
                <table>
...
<!-- User Modal - Shows Current User Details -->
...
<?php if ($show_welcome): ?>
<script>
    // Remove the overlay from DOM after animation completes (10 seconds)
    setTimeout(function() {
...
// Make role badge in header clickable (if exists)
