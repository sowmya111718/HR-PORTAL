...
// ── LOP (Loss of Pay) ───────────────────────────────────
$lop_total          = getLOPCount($conn, $user_id); // This gets LOP from leaves table
...
// For permissions, we'll track them separately but they're typically paid (not LOP)
// If permissions can be LOP in your system, we need to check how it's stored
$permission_total = 0;
$permission_month = 0;
$permission_prev_month = 0;

// Get total permissions (you can modify this if permissions can be LOP)
$perm_query = "SELECT COUNT(*) as total FROM permissions WHERE user_id = ?";
...
// Get permissions this month
$perm_month_query = "SELECT COUNT(*) as total FROM permissions WHERE user_id = ? AND MONTH(permission_date) = MONTH(CURDATE()) AND YEAR(permission_date) = YEAR(CURDATE())";
...
// Get permissions previous month
$perm_prev_query = "SELECT COUNT(*) as total FROM permissions WHERE user_id = ? AND MONTH(permission_date) = MONTH(DATE_SUB(CURDATE(), INTERVAL 1 MONTH)) AND YEAR(permission_date) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 MONTH))";
...
// ── Pending count (including permissions) ─────────────────────────
$pending = 0;
$stmt = $conn->prepare("
    SELECT
...
// ── Recent permissions ────────────────────
$recent_permissions = [];
$recent_perm_stmt = $conn->prepare("SELECT * FROM permissions WHERE user_id = ? ORDER BY applied_date DESC LIMIT 5");
if ($recent_perm_stmt) {
    $recent_perm_stmt->bind_param("i", $user_id);
    $recent_perm_stmt->execute();
    $recent_permissions = $recent_perm_stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    $recent_perm_stmt->close();
}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .paid-badge { background: #276749; color: white; padding: 2px 8px; border-radius: 12px; font-size: 11px; margin-left: 5px; }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card lop-card">
                ...
                <div class="stat-label">LOP — This Month (Leaves)</div>
                <div class="stat-sub">
            ...
            <!-- Permissions (typically paid) -->
            <div class="stat-card" style="background: linear-gradient(135deg, #4299e1 0%, #2b6cb0 100%); color: white;">
                <div class="stat-icon" style="background: rgba(255,255,255,0.2); color: white;"><i class="fas fa-clock"></i></div>
                <div class="stat-value"><?php echo $permission_month; ?></div>
                <div class="stat-label">Permissions — This Month</div>
                <div class="stat-sub" style="border-top-color: rgba(255,255,255,0.2); color: rgba(255,255,255,0.9);">
                    <div>Total: <?php echo $permission_total; ?> permissions</div>
                    <div>Previous month: <?php echo $permission_prev_month; ?></div>
                </div>
            </div>
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-clock"></i> Recent Permissions</h3>
            ...
            <div class="table-container">
                <table>
                    <thead>
                        <tr><th>Date</th><th>Duration</th><th>Reason</th><th>Status</th></tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="permission-row">
                            ...
                            <td>
                                <?php 
                                echo $permission['duration'] == 0.5 ? 'Half Day' : 'Full Day';
                                if ($permission['duration'] == 0.5 && isset($permission['half_type'])) {
                                    echo ' (' . htmlspecialchars($permission['half_type']) . ')';
                                }
                                ?>
                            </td>
                            <td style="max-width:160px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="<?php echo htmlspecialchars($permission['reason'] ?? ''); ?>">
                                <?php echo htmlspecialchars(mb_strimwidth($permission['reason'] ?? '—', 0, 30, '…')); ?>
                            </td>
                            <td>
                                <span class="status-badge status-<?php echo strtolower(htmlspecialchars($permission['status'])); ?>">
