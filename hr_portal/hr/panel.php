...
require_once '../includes/leave_functions.php'; // ADD THIS LINE for leave year calculation
...
if (isset($_GET['approve_leave'])) {
    ...
    if ($role === 'hr') {
        $message = '<div class="alert alert-error">HR managers cannot approve leaves. Only Admins and Project Managers can.</div>';
    } else {
        $stmt = $conn->prepare("
            UPDATE leaves 
        ...
        if ($stmt->execute()) {
            $message = '<div class="alert alert-success">Leave approved successfully</div>';
        } else {
            $message = '<div class="alert alert-error">Error approving leave</div>';
        }
        $stmt->close();
    }
}
...
if (isset($_GET['reject_leave'])) {
    ...
    if ($role === 'hr') {
        $message = '<div class="alert alert-error">HR managers cannot reject leaves. Only Admins and Project Managers can.</div>';
    } else {
        $stmt = $conn->prepare("
            UPDATE leaves 
        ...
        if ($stmt->execute()) {
            $message = '<div class="alert alert-success">Leave rejected successfully</div>';
        } else {
            $message = '<div class="alert alert-error">Error rejecting leave</div>';
        }
        $stmt->close();
    }
}
...
if (isset($_GET['approve_permission'])) {
    ...
    if ($role === 'hr') {
        $message = '<div class="alert alert-error">HR managers cannot approve permissions. Only Admins and Project Managers can.</div>';
    } else {
        $stmt = $conn->prepare("
            UPDATE permissions 
            SET status = 'Approved', approved_by = ?, approved_date = CURDATE() 
        ...
        if ($stmt->execute()) {
            $message = '<div class="alert alert-success">Permission approved successfully</div>';
        } else {
            $message = '<div class="alert alert-error">Error approving permission</div>';
        }
        $stmt->close();
    }
}
...
if (isset($_GET['reject_permission'])) {
    ...
    if ($role === 'hr') {
        $message = '<div class="alert alert-error">HR managers cannot reject permissions. Only Admins and Project Managers can.</div>';
    } else {
        $stmt = $conn->prepare("
            UPDATE permissions 
            SET status = 'Rejected', rejected_by = ?, rejected_date = CURDATE() 
        ...
        if ($stmt->execute()) {
            $message = '<div class="alert alert-success">Permission rejected successfully</div>';
        } else {
            $message = '<div class="alert alert-error">Error rejecting permission</div>';
        }
        $stmt->close();
    }
}
...
// Get filter values
$leave_filter = isset($_GET['leave_filter']) ? sanitize($_GET['leave_filter']) : 'pending';
$permission_filter = isset($_GET['permission_filter']) ? sanitize($_GET['permission_filter']) : 'pending';
$leave_year_filter = isset($_GET['leave_year']) ? sanitize($_GET['leave_year']) : $leave_year['year_label'];

// Get leaves with filter - NOW INCLUDING APPROVED/REJECTED BY INFO AND LEAVE YEAR
$leave_where = "1=1";
if ($leave_filter !== 'all') {
    $leave_where .= " AND l.status = '$leave_filter'";
}
if ($leave_year_filter !== 'all') {
    $leave_where .= " AND l.leave_year = '$leave_year_filter'";
}
...
// Get permissions with filter - NOW INCLUDING APPROVED/REJECTED BY INFO
$permission_where = "1=1";
if ($permission_filter !== 'all') {
    $permission_where = "p.status = '$permission_filter'";
}
...
// Get statistics - UPDATED TO SHOW REAL-TIME COUNTS
        ...
        <div class="main-content">
            <div class="page-title">
                <div class="title-left">
                    <?php if ($role === 'pm'): ?>
                        <i class="fas fa-tasks role-icon role-pm-icon"></i>
                    <?php elseif ($role === 'hr'): ?>
                        <i class="fas fa-users role-icon role-hr-icon"></i>
                    <?php elseif ($role === 'admin'): ?>
                        <i class="fas fa-shield-alt role-icon role-admin-icon"></i>
                    <?php endif; ?>
            ...
            <!-- ADD THIS: Leave Year Information at the top of HR panel -->
            <div class="leave-year-stats">
                ...
                <div class="stats-card">
                    ...
                    <div class="stats-sub">
                        <i class="fas fa-file-alt"></i> <?php echo $leave_stats['current_applications']; ?> applications
                    </div>
                </div>
                <div class="stats-card">
                    <div class="stats-label">Days Until Reset</div>
                    <div class="stats-value"><?php echo $leave_stats['days_until_reset']; ?> days</div>
                    <div class="stats-sub" style="color: #ed8936;">
                        <i class="fas fa-calendar"></i> Reset: <?php echo $leave_stats['reset_date']; ?>
                    </div>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 20px;">
                <div class="stat-card">
                    ...
                    <div style="font-size: 12px; color: #c53030; margin-top: 5px;">
                        <i class="fas fa-users"></i> <?php echo $lop_stats['employees_with_lop']; ?> employees
                    </div>
                </div>
                ...
                <div class="stat-card">
                    <div class="stat-icon" style="background: #feebc8; color: #c05621;"><i class="fas fa-hourglass-half"></i></div>
                    <div class="stat-value"><?php echo $stats['pending_leaves']; ?></div>
                    <div class="stat-label">Pending Leaves</div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon" style="background: #feebc8; color: #c05621;"><i class="fas fa-clock"></i></div>
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 20px;">
                <div class="stat-card">
                    <div class="stat-icon" style="background: #c6f6d5; color: #276749;"><i class="fas fa-calendar-check"></i></div>
                    <div class="stat-value"><?php echo $stats['today_attendance']; ?></div>
                    <div class="stat-label">Today's Attendance</div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon" style="background: #c6f6d5; color: #276749;"><i class="fas fa-umbrella-beach"></i></div>
            ...
            <!-- Today's Approval Stats -->
            <div class="today-stats">
                <div class="today-stat approved">
                    <div class="today-stat-value"><?php echo $stats['today_approved_leaves']; ?></div>
                    <div class="today-stat-label">Leaves Approved Today</div>
                </div>
                ...
                <div class="today-stat approved">
                    <div class="today-stat-value"><?php echo $stats['today_approved_permissions']; ?></div>
                    <div class="today-stat-label">Permissions Approved Today</div>
                </div>
                <div class="today-stat rejected">
            ...
            <div id="exportLeavesModal" class="export-modal">
                <div class="export-modal-content">
                    <h3><i class="fas fa-file-excel"></i> Export Leaves to Excel</h3>
            ...
            <div id="exportPermissionsModal" class="export-modal">
                <div class="export-modal-content">
                    <h3><i class="fas fa-file-excel"></i> Export Permissions to Excel</h3>
                    <form method="POST" action="">
                        <div class="export-options">
                            <div class="export-option-group">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-umbrella-beach"></i> Leave Requests</h3>
                    <div class="export-btn-group">
                        <!-- ADD THIS: Leave Year Filter -->
                        <div class="leave-year-filter">
                            <span class="leave-year-label"><i class="fas fa-calendar-alt"></i> Leave Year:</span>
                                    ...
                                    <td>
                                        <?php if ($is_lop): ?>
                                            <span class="lop-badge">
                                                <i class="fas fa-exclamation-circle"></i> LOP
                                            </span>
                                        <?php else: ?>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="10" style="text-align: center; padding: 40px; color: #718096;">
                                        <i class="fas fa-umbrella-beach" style="font-size: 48px; margin-bottom: 15px; display: block; color: #cbd5e0;"></i>
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-clock"></i> Permission Requests</h3>
                    <div class="export-btn-group">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="7" style="text-align: center; padding: 40px; color: #718096;">
                                        <i class="fas fa-clock" style="font-size: 48px; margin-bottom: 15px; display: block; color: #cbd5e0;"></i>
